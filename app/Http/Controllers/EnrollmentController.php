...
class EnrollmentController extends Controller
{
    ...
    private function submitNewOrTransferEnrollment(Request $request)
    {
        ...
        $validated['student_birthdate'] = now()->subYears(6);
        $validated['student_gender'] = 'male';
        $validated['student_nationality'] = 'Filipino';
        $validated['address'] = 'To be completed';
        $validated['barangay'] = 'To be completed';
        $validated['city'] = 'To be completed';
        $validated['guardian_name'] = 'To be completed';
        $validated['guardian_relationship'] = 'Parent';
        $validated['guardian_contact'] = 'To be completed';
        $validated['emergency_contact_name'] = 'To be completed';
        $validated['emergency_contact_relationship'] = 'Parent';
        $validated['emergency_contact_number'] = 'To be completed';
    ...
    public function adminIndex(Request $request)
    {
        ...
        // Only show CONTINUING student applications for ACTIVE school year
        $query = EnrollmentApplication::with(['gradeLevel', 'schoolYear', 'documents', 'student.user'])
        ...
        // Stats for CONTINUING students only for ACTIVE school year
        $stats = [
            'pending' => EnrollmentApplication::where('application_type', 'continuing')->where('school_year_id', $activeSchoolYear?->id)->pending()->count(),
            'under_review' => EnrollmentApplication::where('application_type', 'continuing')->where('school_year_id', $activeSchoolYear?->id)->underReview()->count(),
            'approved' => EnrollmentApplication::where('application_type', 'continuing')->where('school_year_id', $activeSchoolYear?->id)->approved()->count(),
            'rejected' => EnrollmentApplication::where('application_type', 'continuing')->where('school_year_id', $activeSchoolYear?->id)->rejected()->count(),
            'total' => EnrollmentApplication::where('application_type', 'continuing')->where('school_year_id', $activeSchoolYear?->id)->count(),
        ];

        // Get all sections for bulk assignment
        $allSections = Section::with('gradeLevel')->where('is_active', true)->orderBy('name')->get();

        return view('enrollment.admin.index', compact('applications', 'stats', 'allSections', 'activeSchoolYear'));
    }
    ...
    private function updateContinuingStudentEnrollment(EnrollmentApplication $application): void
    {
        ...
        if ($student) {
            ...
            // TODO: Create new enrollment record for the school year if needed
        }
    }
