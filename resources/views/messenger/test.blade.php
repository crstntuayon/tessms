<!DOCTYPE html>
<html>
<head>
    <title>Messenger Test</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
</head>
<body class="bg-gray-100 p-8">
    <div class="max-w-4xl mx-auto">
        <h1 class="text-2xl font-bold mb-4">Messenger Test</h1>
        
        <div class="bg-white p-4 rounded-lg shadow mb-4">
            <h2 class="font-semibold mb-2">User Info:</h2>
            <p>Name: {{ auth()->user()->full_name ?? 'Not logged in' }}</p>
            <p>Role: {{ auth()->user()->role->name ?? 'No role' }}</p>
            <p>ID: {{ auth()->id() }}</p>
        </div>
        
        <div class="bg-white p-4 rounded-lg shadow mb-4">
            <h2 class="font-semibold mb-2">Contacts ({{ $contacts->count() }}):</h2>
            @forelse($contacts as $contact)
                <div class="p-2 border-b">
                    <p class="font-medium">{{ $contact->full_name }}</p>
                    <p class="text-sm text-gray-500">ID: {{ $contact->id }}</p>
                </div>
            @empty
                <p class="text-gray-500">No contacts found</p>
            @endforelse
        </div>
        
        <a href="{{ $isStudent ? route('student.messenger') : route('teacher.messenger') }}" 
           class="inline-block bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">
            Go to Messenger
        </a>
    </div>
</body>
</html>
