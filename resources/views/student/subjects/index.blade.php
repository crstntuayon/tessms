...
<body class="bg-slate-100 min-h-screen overflow-x-hidden"
    ...
    <!-- Main Content -->
    <main id="mainContent" 
          class="min-h-screen p-4 lg:p-6 transition-all duration-300"
          :class="{ 
        ...
        <div class="load-slip bg-white p-4 lg:p-6 rounded-lg shadow-lg relative overflow-hidden">
            ...
            <div class="relative z-10">
                <div class="flex flex-col lg:flex-row items-center lg:items-start justify-between mb-2 gap-4">
                    ...
                    <div class="text-center lg:text-right">
                        <div class="bg-indigo-900 text-white px-4 py-2 rounded-lg inline-block">
                            ...
                            <p class="text-lg font-bold">{{ $schoolYear }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="header-line flex flex-col sm:flex-row items-center justify-between gap-2">
                    ...
                    <span class="status-badge {{ $isEnrolled ? 'status-enrolled' : 'status-pending' }}">
            ...
            <div class="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-2 mb-6">
                <div class="info-row">
                    ...
                    <span class="info-value font-mono text-base">{{ $student->lrn ?? 'N/A' }}</span>
                </div>
                ...
                <div class="info-row">
                    ...
                    <span class="info-value font-semibold">{{ $gradeLevel->name ?? 'N/A' }}</span>
                </div>
                ...
                <div class="info-row">
                    ...
                    <span class="info-value font-mono">{{ $section->room_number ?? 'TBA' }}</span>
                </div>
                
                <div class="info-row">
                    <span class="info-label">Adviser:</span>
                    <span class="info-value">
                        @if($section && $section->teacher && $section->teacher->user)
                            {{ $section->teacher->user->last_name ?? '' }}, {{ $section->teacher->user->first_name ?? '' }} {{ $section->teacher->user->middle_name ?? '' }}
                        @else
                            N/A
                        @endif
                    </span>
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $enrollment->created_at ? $enrollment->created_at->format('F d, Y h:i A') : now()->format('F d, Y h:i A') }}</span>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($subjects as $index => $subject)
                            <tr>
                                ...
                                <td class="text-center font-mono font-semibold">{{ $subject->code ?? 'N/A' }}</td>
                                <td class="text-center font-semibold">{{ $section->name ?? 'N/A' }}</td>
                                <td>{{ $subject->name }}</td>
                                <td class="text-center">{{ $subject->schedule ?? 'TBA' }}</td>
                                <td class="text-center font-mono">{{ $section->room_number ?? 'TBA' }}</td>
                            </tr>
                        @empty
                            <tr>
            ...
            <div class="relative z-10 border-t-2 border-indigo-900 pt-3 mt-4">
                <div class="flex flex-col lg:flex-row justify-between items-center gap-4 text-[10px] text-slate-600">
                    <div class="flex flex-col lg:flex-row gap-2 lg:gap-6 text-center lg:text-left">
                        <p><span class="font-semibold">Encoded By:</span> {{ $enrollment->encoded_by ?? 'System Administrator' }}</p>
                        <p><span class="font-semibold">Printed By:</span> {{ auth()->user()->name ?? 'System' }}</p>
                        <p><span class="font-semibold">Date Printed:</span> {{ now()->format('F d, Y h:i A') }}</p>
                    </div>
                    <div class="flex gap-4">
                        <div class="bg-indigo-50 px-4 py-2 rounded-lg">
                            <span class="font-semibold text-indigo-900">Total Subjects: {{ $totalSubjects }}</span>
                        </div>
                    </div>
                </div>
            </div>
