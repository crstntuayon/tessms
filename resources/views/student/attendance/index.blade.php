...
<body class="bg-slate-50 min-h-screen"
    ...
    <main class="min-h-screen transition-all duration-300 ease-out p-4 lg:p-8"
          :class="{ 
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="stat-card border-l-4 border-emerald-500">
                ...
                <div class="mt-3 text-xs text-emerald-600 font-medium">
                    {{ $stats['total_school_days'] > 0 ? round(($stats['present'] / $stats['total_school_days']) * 100, 1) : 0 }}% of school days
                </div>
            </div>
            ...
            <div class="stat-card border-l-4 border-rose-500">
                ...
                <div class="mt-3 text-xs text-rose-600 font-medium">
                    {{ $stats['total_school_days'] > 0 ? round(($stats['absent'] / $stats['total_school_days']) * 100, 1) : 0 }}% of school days
                </div>
            </div>
            ...
            <div class="stat-card border-l-4 border-amber-500">
                ...
                <div class="mt-3 text-xs text-amber-600 font-medium">
                    {{ $stats['total_school_days'] > 0 ? round(($stats['late'] / $stats['total_school_days']) * 100, 1) : 0 }}% of school days
                </div>
            </div>
            ...
            <div class="p-6 border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-white">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('student.attendance', ['month' => $selectedDate->copy()->subMonth()->month, 'year' => $selectedDate->copy()->subMonth()->year]) }}" 
                        ...
                        <a href="{{ route('student.attendance', ['month' => \Carbon\Carbon::now()->month, 'year' => \Carbon\Carbon::now()->year]) }}" 
                        ...
                        <a href="{{ route('student.attendance', ['month' => $selectedDate->copy()->addMonth()->month, 'year' => $selectedDate->copy()->addMonth()->year]) }}" 
                           class="calendar-nav-btn w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-600">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-7 gap-2">
                    ...
                    @for ($i = 0; $i < $totalCells; $i++)
                        ...
                        <div class="calendar-day rounded-xl border-2 {{ $isCurrentMonth ? 'bg-white border-slate-100' : 'bg-slate-50 border-transparent' }} 
                                    {{ $attendance ? 'status-' . $attendance->status : '' }} 
                                    {{ $isToday ? 'ring-2 ring-indigo-500 ring-offset-2' : '' }}
                                    {{ $isWeekend ? 'bg-slate-50' : '' }}
                                    p-3 relative cursor-pointer"
                             @if($isCurrentMonth) @click="showAttendanceDetails('{{ $cellDate->format('Y-m-d') }}')" @endif>
                            
                            @if($isCurrentMonth)
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-semibold {{ $isToday ? 'text-indigo-600' : 'text-slate-700' }}">
                                        {{ $dayCounter - 1 }}
                                    </span>
                                    @if($isToday)
                                ...
                                @if($attendance)
                                    ...
                                    @if($attendance->remarks)
                                        <p class="text-xs text-slate-500 mt-1 truncate">{{ $attendance->remarks }}</p>
                                    @endif
                                @elseif(!$isWeekend)
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-3">
                    <div class="flex justify-between items-center p-3 bg-emerald-50 rounded-xl">
                        ...
                        <span class="text-emerald-900 font-bold text-lg">{{ $stats['total_school_days'] }}</span>
                    </div>
                    <div class="flex justify-between items-center p-3 bg-blue-50 rounded-xl">
                        <span class="text-blue-700 font-medium">Days Attended</span>
                        <span class="text-blue-900 font-bold text-lg">{{ $stats['attended_days'] }}</span>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center p-3 bg-rose-50 rounded-xl">
                        ...
                        <span class="text-rose-900 font-bold text-lg">{{ $stats['absent'] }}</span>
                    </div>
                    <div class="flex justify-between items-center p-3 bg-amber-50 rounded-xl">
                        ...
                        <span class="text-amber-900 font-bold text-lg">{{ $stats['late'] }}</span>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-center">
                    <div class="text-center">
                        <div class="text-4xl font-bold text-indigo-600 mb-1">
                            {{ $stats['total_school_days'] > 0 ? round(($stats['attended_days'] / $stats['total_school_days']) * 100, 1) : 0 }}%
                        </div>
