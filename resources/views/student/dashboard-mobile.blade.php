...
<head>
    ...
    <title>Dashboard - {{ auth()->user()->student->first_name ?? 'Student' }}</title>
    ...
    <div x-data="studentDashboard()" x-init="init()" class="min-h-screen bg-slate-50 pb-24">
        ...
        <div class="bg-gradient-to-br from-blue-600 to-purple-700 text-white safe-top">
            <div class="px-5 pt-6 pb-8">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <p class="text-blue-100 text-sm">Good {{ $greeting ?? 'morning' }},</p>
                        <h1 class="text-2xl font-bold">{{ auth()->user()->first_name ?? 'Student' }}</h1>
                    </div>
                    <div class="flex items-center space-x-3">
                        ...
                        <a href="{{ route('student.profile') }}" class="w-10 h-10 rounded-full bg-white/30 flex items-center justify-center overflow-hidden">
                            ...
                            @else
                                <span class="font-bold">{{ strtoupper(substr(auth()->user()->first_name ?? 'S', 0, 1)) }}</span>
                            @endif
                        </a>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div class="bg-white/20 backdrop-blur rounded-2xl p-3 text-center">
                        ...
                        <p class="text-2xl font-bold">{{ $attendanceRate ?? '95' }}%</p>
                    </div>
                    <div class="bg-white/20 backdrop-blur rounded-2xl p-3 text-center">
                        <p class="text-blue-100 text-xs mb-1">Average</p>
                        <p class="text-2xl font-bold">{{ $averageGrade ?? '92' }}</p>
                    </div>
                    <div class="bg-white/20 backdrop-blur rounded-2xl p-3 text-center">
                        <p class="text-blue-100 text-xs mb-1">Rank</p>
                        <p class="text-2xl font-bold">{{ $classRank ?? '5' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 -mt-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <span class="text-sm text-slate-500">{{ now()->format('l, M d') }}</span>
                </div>
                <div class="space-y-2">
                    @forelse($todaySchedule ?? [] as $subject)
                    <div class="flex items-center space-x-3 p-3 rounded-xl {{ $subject['status'] === 'completed' ? 'bg-slate-50' : 'bg-blue-50' }}">
                        <div class="w-12 h-12 rounded-xl {{ $subject['status'] === 'completed' ? 'bg-slate-200' : 'bg-blue-500' }} flex items-center justify-center text-white font-bold text-sm">
                            {{ substr($subject['name'], 0, 2) }}
                        </div>
                        <div class="flex-1">
                            <p class="font-semibold text-slate-800">{{ $subject['name'] }}</p>
                            <p class="text-sm text-slate-500">{{ $subject['time'] }}</p>
                        </div>
                        @if($subject['status'] === 'completed')
                            <i class="fas fa-check-circle text-green-500 text-xl"></i>
                        @elseif($subject['status'] === 'current')
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('student.grades') }}" class="text-sm text-blue-600 font-medium">View All</a>
                </div>
                <div class="space-y-2">
                    @forelse($recentGrades ?? [] as $grade)
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                        <div>
                            <p class="font-semibold text-slate-800">{{ $grade['subject'] }}</p>
                            <p class="text-sm text-slate-500">{{ $grade['type'] }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xl font-bold {{ $grade['score'] >= 75 ? 'text-green-600' : 'text-red-600' }}">{{ $grade['score'] }}%</p>
                            <p class="text-xs text-slate-400">{{ $grade['date'] }}</p>
                        </div>
                    </div>
                    @empty
            ...
            <div class="grid grid-cols-4 gap-3 mb-4">
                <a href="{{ route('student.grades') }}" class="flex flex-col items-center p-3 bg-white rounded-2xl shadow-sm hover:shadow-md transition">
                ...
                <a href="{{ route('student.attendance') }}" class="flex flex-col items-center p-3 bg-white rounded-2xl shadow-sm hover:shadow-md transition">
                ...
                <a href="{{ route('student.announcements') }}" class="flex flex-col items-center p-3 bg-white rounded-2xl shadow-sm hover:shadow-md transition">
                ...
                <a href="{{ route('student.messenger') }}" class="flex flex-col items-center p-3 bg-white rounded-2xl shadow-sm hover:shadow-md transition">
            ...
            <div class="bg-white rounded-2xl shadow-sm p-4 mb-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('student.announcements') }}" class="text-sm text-blue-600 font-medium">View All</a>
                </div>
                <div class="space-y-3">
                    @forelse($recentAnnouncements ?? [] as $announcement)
                    <a href="{{ route('student.announcements.show', $announcement['id']) }}" class="block p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition">
                        <div class="flex items-start space-x-3">
                            <div class="w-10 h-10 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                <i class="fas fa-bullhorn"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-slate-800 truncate">{{ $announcement['title'] }}</p>
                                <p class="text-sm text-slate-500 line-clamp-2">{{ $announcement['excerpt'] }}</p>
                                <p class="text-xs text-slate-400 mt-1">{{ $announcement['date'] }}</p>
                            </div>
                        </div>
                    </a>
                    @empty
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center space-x-3 p-3 bg-red-50 rounded-xl border border-red-100">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-red-600">Due {{ $assignment['due_date'] }}</p>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 safe-bottom z-40">
            <div class="flex justify-around py-2">
                <a href="{{ route('student.dashboard') }}" class="flex flex-col items-center p-2 text-blue-600">
                    <i class="fas fa-home text-xl mb-1"></i>
                    <span class="text-[10px] font-medium">Home</span>
                </a>
                <a href="{{ route('student.subjects') }}" class="flex flex-col items-center p-2 text-slate-400 hover:text-slate-600">
                    <i class="fas fa-book text-xl mb-1"></i>
                    <span class="text-[10px] font-medium">Subjects</span>
                </a>
                <a href="{{ route('student.messenger') }}" class="flex flex-col items-center p-2 text-slate-400 hover:text-slate-600">
                    <i class="fas fa-comment text-xl mb-1"></i>
                    <span class="text-[10px] font-medium">Chat</span>
                </a>
                <a href="{{ route('student.profile') }}" class="flex flex-col items-center p-2 text-slate-400 hover:text-slate-600">
        ...
        <div x-show="showNotifications" 
            ...
            <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl max-h-[80vh] overflow-hidden" @click.stop>
                ...
                <div class="overflow-y-auto max-h-[60vh] p-4 space-y-3">
                    ...
                    <div class="p-3 bg-slate-50 rounded-xl {{ $notification['read'] ? '' : 'border-l-4 border-blue-500' }}">
