...
<head>
    ...
    <title>New Message - Student Portal</title>
...
<body class="min-h-screen bg-slate-50 font-sans antialiased"
      x-data="{ 
          ...
          mobileOpen: false,
          showTeachers: {{ $gradeLevel && in_array($gradeLevel->name, ['Grade 5', 'Grade 6', '5', '6']) ? 'true' : 'false' }}
      }"
    ...
    <main class="min-h-screen transition-all duration-300" :class="sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-72'">
        ...
        <div class="max-w-2xl mx-auto p-4">
            ...
            <form action="{{ route('student.messages.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-slate-200">
                @csrf
                ...
                <div class="p-4 border-b border-slate-100">
                    ...
                    @if($gradeLevel && in_array($gradeLevel->name, ['Grade 5', 'Grade 6', '5', '6']))
                        {{-- Grade 5-6: Can select different teachers --}}
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-all" 
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-slate-500">{{ $defaultTeacher ? $defaultTeacher->full_name : 'Not assigned' }}</p>
                                </div>
                            </label>
                        ...
                        {{-- Teacher Select (shown only when "Other" selected) --}}
                        ...
                        {{-- Hidden input for adviser --}}
                        <input x-show="!showTeachers" type="hidden" name="recipient_id" value="{{ $defaultTeacher ? $defaultTeacher->id : '' }}">
                    @else
                        {{-- Grade 1-4: Default to adviser only --}}
                        <div class="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                            <div class="w-10 h-10 rounded-full bg-indigo-500 flex items-center justify-center text-white font-semibold">
                                {{ $defaultTeacher ? strtoupper(substr($defaultTeacher->first_name, 0, 1)) : '?' }}
                            </div>
                            <div class="flex-1">
                                <p class="font-medium text-slate-800">{{ $defaultTeacher ? $defaultTeacher->full_name : 'Not assigned' }}</p>
                                <p class="text-sm text-slate-500">Your Class Adviser</p>
                            </div>
                        </div>
                        <input type="hidden" name="recipient_id" value="{{ $defaultTeacher ? $defaultTeacher->id : '' }}">
                    @endif
                </div>

                {{-- Optional Subject (for context) --}}
                <div class="px-4 pt-4">
                ...
                <div class="p-4 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('student.messages.index') }}" class="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg text-sm font-medium transition-all">
                        Cancel
                    </a>
