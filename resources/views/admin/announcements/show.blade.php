...
@section('header-title', 'Announcement Details')

@section('content')
<div class="max-w-3xl mx-auto space-y-6" x-data="announcementShow()">
    {{-- Main Card --}}
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="px-6 py-3 {{ $announcement->priority === 'urgent' ? 'bg-rose-50 border-b border-rose-100' : ($announcement->priority === 'important' ? 'bg-amber-50 border-b border-amber-100' : 'bg-slate-50 border-b border-slate-100') }}">
            <div class="flex items-center gap-2">
                ...
                @if($announcement->pinned)
                    <span class="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded-full"><i class="fas fa-thumbtack mr-1"></i>Pinned</span>
                @endif
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">{{ $announcement->author?->full_name ?? 'Unknown' }}</p>
                ...
                <div class="mt-6 pt-6 border-t border-slate-100">
                    ...
                    <div class="flex flex-wrap gap-3">
                        @foreach($announcement->attachments as $att)
                            ...
                            @else
                                <a href="{{ $att->url() }}" download class="flex items-center gap-2 px-4 py-3 bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-200 rounded-xl text-sm text-slate-700 hover:text-blue-700 transition-all">
        ...
        <div class="p-6">
            ...
            @else
                <div class="text-center py-8 text-slate-400">
                    ...
                    <p class="text-sm">Read statistics are only available for section and grade-level announcements.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 space-y-3 text-sm">
            <div class="flex justify-between"><span class="text-slate-500">Scope</span><span class="font-medium text-slate-900 capitalize">{{ $announcement->scope }}</span></div>
            @if($announcement->scope === 'section' && $announcement->section)
                <div class="flex justify-between"><span class="text-slate-500">Target Section</span><span class="font-medium text-slate-900">{{ $announcement->section->name }} ({{ $announcement->section->gradeLevel->name ?? 'N/A' }})</span></div>
            @endif
            @if($announcement->scope === 'grade' && $announcement->gradeLevel)
                <div class="flex justify-between"><span class="text-slate-500">Target Grade</span><span class="font-medium text-slate-900">{{ $announcement->gradeLevel->name }}</span></div>
            @endif
            <div class="flex justify-between"><span class="text-slate-500">Posted On</span><span class="font-medium text-slate-900">{{ $announcement->created_at->format('M d, Y g:i A') }}</span></div>
            @if($announcement->expires_at)
                <div class="flex justify-between"><span class="text-slate-500">Expires</span><span class="font-medium {{ $announcement->expires_at->isPast() ? 'text-rose-600' : 'text-slate-900' }}">{{ $announcement->expires_at->format('M d, Y g:i A') }}</span></div>
            @endif
            <div class="flex justify-between"><span class="text-slate-500">School Year</span><span class="font-medium text-slate-900">{{ $announcement->schoolYear->name ?? 'N/A' }}</span></div>
        </div>
    </div>

    {{-- Action Buttons --}}
    <div class="flex items-center justify-end gap-3">
        <a href="{{ route('admin.announcements.index') }}" class="px-5 py-2.5 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors"><i class="fas fa-arrow-left mr-1"></i> Back</a>
        <a href="{{ route('admin.announcements.edit', $announcement) }}" class="px-5 py-2.5 text-sm font-medium text-emerald-600 bg-emerald-50 rounded-xl hover:bg-emerald-100 transition-colors"><i class="fas fa-edit mr-1"></i> Edit</a>
        <button @click="confirmDelete()" class="px-5 py-2.5 text-sm font-medium text-rose-600 bg-rose-50 rounded-xl hover:bg-rose-100 transition-colors"><i class="fas fa-trash mr-1"></i> Delete</button>
    </div>
...
<div x-show="deleteModalOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm" x-transition.opacity>
    <div class="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4 transform transition-all" x-transition.scale>
        <div class="text-center">
            <div class="w-16 h-16 bg-rose-100 rounded-full flex items-center justify-center mx-auto mb-4"><i class="fas fa-exclamation-triangle text-2xl text-rose-500"></i></div>
            ...
            <div class="flex gap-3">
                ...
                <button @click="executeDelete()" :disabled="deleteCountdown > 0" class="flex-1 px-4 py-2.5 bg-rose-500 text-white rounded-xl font-medium hover:bg-rose-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2">
                    <template x-if="deleteCountdown > 0"><span class="flex items-center gap-1.5"><i class="fas fa-clock text-sm"></i><span x-text="deleteCountdown + 's'"></span></span></template>
...
<div x-show="lightboxOpen" x-cloak @keydown.escape.window="lightboxOpen = false" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/90" x-transition.opacity>
    <button @click="lightboxOpen = false" class="absolute top-4 right-4 p-3 text-white/70 hover:text-white transition-colors z-10"><i class="fas fa-times text-2xl"></i></button>
    <a :href="lightboxImage" download class="absolute top-4 left-4 p-3 text-white/70 hover:text-white transition-colors z-10" title="Download image"><i class="fas fa-download text-xl"></i></a>
    <img :src="lightboxImage" class="max-w-[95vw] max-h-[90vh] object-contain rounded-lg shadow-2xl" @click.stop>
</div>

<form x-ref="deleteForm" action="{{ route('admin.announcements.destroy', $announcement) }}" method="POST" class="hidden">@csrf @method('DELETE')</form>
</div>
    ...
    return {
        ...
        confirmDelete() {
            ...
            this.deleteCountdownInterval = setInterval(() => {
                ...
                if (this.deleteCountdown <= 0) { clearInterval(this.deleteCountdownInterval); this.deleteCountdownInterval = null; }
            }, 1000);
        },
        cancelDelete() {
            this.deleteModalOpen = false;
            if (this.deleteCountdownInterval) { clearInterval(this.deleteCountdownInterval); this.deleteCountdownInterval = null; }
        },
        async executeDelete() {
            if (this.deleteCountdown > 0) return;
            try {
                const token = document.querySelector('meta[name="csrf-token"]').content;
                const res = await fetch('{{ route('admin.announcements.destroy', $announcement) }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': token,
