...
@section('header-title', 'School Announcements')
...
<div x-data="announcementList()" class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.announcements.create') }}" class="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg shadow-blue-500/30">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
            ...
            <p class="text-2xl font-bold text-slate-900 mt-1">{{ $announcements->total() }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
            <p class="text-xs font-semibold text-amber-600 uppercase tracking-wider">Pinned</p>
            <p class="text-2xl font-bold text-slate-900 mt-1">{{ $announcements->where('pinned', true)->count() }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
            <p class="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Total Reads</p>
            <p class="text-2xl font-bold text-slate-900 mt-1">{{ $announcements->sum('reads_count') }}</p>
        </div>
        <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
            <p class="text-xs font-semibold text-rose-600 uppercase tracking-wider">Urgent</p>
            <p class="text-2xl font-bold text-slate-900 mt-1">{{ $announcements->where('priority', 'urgent')->count() }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        @if($announcements->isEmpty())
            <div class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p class="text-sm text-slate-400 mt-1">Create an announcement to reach students and teachers.</p>
                <a href="{{ route('admin.announcements.create') }}" class="mt-4 px-5 py-2.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors">
        ...
        @else
            <div class="divide-y divide-slate-100">
                @foreach($announcements as $announcement)
                    <div class="p-5 hover:bg-slate-50 transition-colors {{ $announcement->pinned ? 'bg-amber-50/30' : '' }}">
                        <div class="flex items-start gap-4">
                                ...
                                <div class="flex items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    @if($announcement->author)
                                        <span class="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-medium rounded-full">
                                            by {{ $announcement->author->full_name ?? $announcement->author->name }}
                                        </span>
                                    @endif
                                </div>
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <a href="{{ route('admin.announcements.show', $announcement) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="View Details">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.announcements.edit', $announcement) }}" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Edit">
