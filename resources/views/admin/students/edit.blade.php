        ...
        <div class="relative">
            <div id="photoPreview" class="w-32 h-32 rounded-full bg-slate-100 border-4 border-white shadow-lg flex items-center justify-center overflow-hidden">
                @if($student->user->photo)
                    <img src="{{ asset('storage/' . $student->user->photo) }}" class="w-full h-full object-cover" alt="Current Photo">
                @else
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (photoUrl && photoUrl.trim() !== '') {
        profileContainer.innerHTML = `<img src="/storage/${photoUrl}" class="w-10 h-10 rounded-full object-cover border-2 border-indigo-500 shadow-md" alt="">`;
    } else {
        const initials = getInitials(studentName);
        profileContainer.innerHTML = `<div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm border-2 border-white shadow-md">${initials}</div>`;
    }
