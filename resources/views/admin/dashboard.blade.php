        ...
        <div class="main-wrapper">
            ...
            <main class="main-content">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    ...
                    <!-- Total Students -->
                    <div class="stat-card glass-card rounded-2xl p-4 sm:p-6 shadow-sm animate-fade-in-up" style="animation-delay: 0.1s;">
                        @php
                            ...
                            $lastMonthCount = $activeSchoolYearId
                                ? \App\Models\Enrollment::where('school_year_id', $activeSchoolYearId)
                                    ->whereIn('status', ['approved', 'enrolled', 'completed'])
                                    ->whereMonth('created_at', now()->subMonth()->month)
                                    ->count()
                                : \App\Models\Student::where('status', 'active')->whereMonth('created_at', now()->subMonth()->month)->count();
                            
                            $thisMonthCount = $activeSchoolYearId
                                ? \App\Models\Enrollment::where('school_year_id', $activeSchoolYearId)
                                    ->whereIn('status', ['approved', 'enrolled', 'completed'])
                                    ->whereMonth('created_at', now()->month)
                                    ->count()
                                : \App\Models\Student::where('status', 'active')->whereMonth('created_at', now()->month)->count();
                            
                            $growth = $lastMonthCount > 0 ? round((($thisMonthCount - $lastMonthCount) / $lastMonthCount) * 100, 1) : 0;
                            
                            $maleStudents = $activeSchoolYearId
                                ? \App\Models\Enrollment::where('school_year_id', $activeSchoolYearId)
                                    ->whereIn('status', ['approved', 'enrolled', 'completed'])
                                    ->whereHas('student', function($sq) { $sq->where('gender', 'Male'); })->count()
                                : \App\Models\Student::where('status', 'active')->where('gender', 'Male')->count();
                            
                            $femaleStudents = $activeSchoolYearId
                                ? \App\Models\Enrollment::where('school_year_id', $activeSchoolYearId)
                                    ->whereIn('status', ['approved', 'enrolled', 'completed'])
                                    ->whereHas('student', function($sq) { $sq->where('gender', 'Female'); })->count()
                                : \App\Models\Student::where('status', 'active')->where('gender', 'Female')->count();
                        @endphp
                        <div class="flex items-start justify-between mb-3 sm:mb-4">
                            <div class="p-2.5 sm:p-3 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl sm:rounded-2xl shadow-lg shadow-blue-500/30">
                                <i class="fas fa-users text-white text-lg sm:text-xl"></i>
                            </div>
                            <div class="flex items-center gap-1 {{ $growth >= 0 ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50' }} px-2 py-1 rounded-full text-xs font-bold border {{ $growth >= 0 ? 'border-emerald-200' : 'border-red-200' }}">
                                <i class="fas fa-arrow-{{ $growth >= 0 ? 'up' : 'down' }} text-[10px]"></i>
                                <span>{{ abs($growth) }}%</span>
                            </div>
                        </div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">{{ number_format($totalStudents) }}</h3>
                        <p class="text-sm text-slate-500 font-medium mb-3 sm:mb-4">Total Students</p>
                        <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs">
                            <div class="flex items-center gap-1.5">
                                <div class="w-2 h-2 rounded-full bg-blue-500"></div>
                                <span class="text-slate-600 font-medium">Male: {{ $maleStudents }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <div class="w-2 h-2 rounded-full bg-pink-500"></div>
                                <span class="text-slate-600 font-medium">Female: {{ $femaleStudents }}</span>
                            </div>
                        </div>
                        @php
                            $schoolCapacity = 2000;
                        ...
                        @endphp
                        <div class="mt-3 sm:mt-4 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div class="h-full bg-blue-500 rounded-full progress-bar" style="width: {{ $percentage }}%"></div>
                        </div>
                    </div>

                    <!-- Total Teachers -->
                    <div class="stat-card glass-card rounded-2xl p-4 sm:p-6 shadow-sm animate-fade-in-up" style="animation-delay: 0.2s;">
                        @php
                        ...
                        @endphp
                        <div class="flex items-start justify-between mb-3 sm:mb-4">
                            <div class="p-2.5 sm:p-3 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl sm:rounded-2xl shadow-lg shadow-purple-500/30">
                                <i class="fas fa-chalkboard-teacher text-white text-lg sm:text-xl"></i>
                            </div>
                            <div class="flex items-center gap-1 text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full text-xs font-bold border border-emerald-200">
                                <i class="fas fa-check text-[10px]"></i>
                                <span>{{ $activePercentage }}% Active</span>
                            </div>
                        </div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">{{ number_format($totalTeachers) }}</h3>
                        <p class="text-sm text-slate-500 font-medium mb-3 sm:mb-4">Total Teachers</p>
                        <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs">
                            <div class="flex items-center gap-1.5">
                                <div class="w-2 h-2 rounded-full bg-emerald-500"></div>
                                <span class="text-slate-600 font-medium">Active: {{ $activeTeachers }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <div class="w-2 h-2 rounded-full bg-slate-400"></div>
                                <span class="text-slate-600 font-medium">Inactive: {{ $inactiveTeachers }}</span>
                            </div>
                        </div>
                        <div class="mt-3 sm:mt-4 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    ...
                    <!-- Today's Attendance -->
                    <div class="stat-card glass-card rounded-2xl p-4 sm:p-6 shadow-sm animate-fade-in-up" style="animation-delay: 0.3s;">
                        @php
                        ...
                        @endphp
                        <div class="flex items-start justify-between mb-3 sm:mb-4">
                            <div class="p-2.5 sm:p-3 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl sm:rounded-2xl shadow-lg shadow-emerald-500/30">
                                <i class="fas fa-calendar-check text-white text-lg sm:text-xl"></i>
                            </div>
                            <div class="flex items-center gap-1 {{ $attendanceRate >= 90 ? 'text-emerald-600 bg-emerald-50' : 'text-amber-600 bg-amber-50' }} px-2 py-1 rounded-full text-xs font-bold border {{ $attendanceRate >= 90 ? 'border-emerald-200' : 'border-amber-200' }}">
                                <i class="fas fa-check text-[10px]"></i>
                                <span>{{ $attendanceRate }}%</span>
                            </div>
                        </div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">{{ $attendanceRate }}%</h3>
                        <p class="text-sm text-slate-500 font-medium mb-3 sm:mb-4">Today's Attendance</p>
                        <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs">
                            <div class="flex items-center gap-1.5">
                                <div class="w-2 h-2 rounded-full bg-emerald-500"></div>
                    ...
                    <!-- Average Grade -->
                    <div class="stat-card glass-card rounded-2xl p-4 sm:p-6 shadow-sm animate-fade-in-up" style="animation-delay: 0.4s;">
                        @php
                            ...
                            $above90 = \App\Models\Grade::where('component_type', 'final_grade')->where('final_grade', '>=', 90)
                                ->when($activeSchoolYearId, function($q) use ($activeSchoolYearId) {
                                    return $q->whereHas('student.enrollments', function($eq) use ($activeSchoolYearId) {
                                        $eq->where('school_year_id', $activeSchoolYearId)->whereIn('status', ['approved', 'enrolled', 'completed']);
                                    });
                            ...
                            $below75 = \App\Models\Grade::where('component_type', 'final_grade')->where('final_grade', '<', 75)
                                ->when($activeSchoolYearId, function($q) use ($activeSchoolYearId) {
                                    return $q->whereHas('student.enrollments', function($eq) use ($activeSchoolYearId) {
                                        $eq->where('school_year_id', $activeSchoolYearId)->whereIn('status', ['approved', 'enrolled', 'completed']);
                                    });
                        ...
                        @endphp
                        <div class="flex items-start justify-between mb-3 sm:mb-4">
                            <div class="p-2.5 sm:p-3 bg-gradient-to-br from-amber-500 to-orange-600 rounded-xl sm:rounded-2xl shadow-lg shadow-amber-500/30">
                                <i class="fas fa-star text-white text-lg sm:text-xl"></i>
                            </div>
                            <div class="flex items-center gap-1 text-amber-600 bg-amber-50 px-2 py-1 rounded-full text-xs font-bold border border-amber-200">
                                <i class="fas fa-chart-line text-[10px]"></i>
                                <span>{{ number_format($avgGrade, 1) }}</span>
                            </div>
                        </div>
                        <h3 class="text-2xl sm:text-3xl font-bold text-slate-900 mb-1">{{ number_format($avgGrade, 1) }}</h3>
                        <p class="text-sm text-slate-500 font-medium mb-3 sm:mb-4">Average Grade</p>
                        <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs">
                            <div class="flex items-center gap-1.5">
