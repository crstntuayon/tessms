<div class="sf4-container bg-white p-4 rounded-xl shadow-lg border border-slate-200 max-w-[1600px] mx-auto">
    <style>
        ...
        <tbody>
            ...
            @forelse($maleData as $index => $data)
                ...
                <tr>
                    ...
                    <td class="font-bold {{ $data['attendance_rate'] >= 90 ? 'text-emerald-600' : ($data['attendance_rate'] >= 75 ? 'text-amber-600' : 'text-rose-600') }} text-[9px]">{{ $data['attendance_rate'] }}%</td>
            ...
            <tr class="bg-gray-50 font-bold">
                ...
                <td class="text-[9px]">{{ $maleData->count() > 0 ? round($maleData->avg('attendance_rate'), 1) : 0 }}%</td>
                <td></td>
            </tr>
            ...
            <tr class="bg-gray-50 font-bold">
                ...
                <td class="text-[9px]">{{ $femaleData->count() > 0 ? round($femaleData->avg('attendance_rate'), 1) : 0 }}%</td>
                <td></td>
            </tr>
            <!-- COMBINED TOTAL ROW -->
            <tr class="bg-gray-200 font-bold border-t-2 border-black">
                ...
                <td class="text-[9px] border-b-2 border-black">{{ number_format($monthlyStats['overall_avg_attendance'] ?? 0, 1) }}%</td>
        ...
        <div class="space-y-2">
            <div class="summary-box bg-gray-50">
                ...
                <div class="space-y-1 text-[8px]">
                    <div class="flex justify-between border-b border-gray-300 pb-1"><span>Total School Days:</span><span class="font-bold">{{ $monthlyStats['total_school_days'] ?? 0 }}</span></div>
                    <div class="flex justify-between border-b border-gray-300 pb-1"><span>Total Enrolled Students:</span><span class="font-bold">{{ $monthlyStats['total_students'] ?? 0 }}</span></div>
                    <div class="flex justify-between border-b border-gray-300 pb-1"><span>Male Students:</span><span class="font-bold">{{ $monthlyStats['male_count'] ?? 0 }}</span></div>
                    <div class="flex justify-between border-b border-gray-300 pb-1"><span>Female Students:</span><span class="font-bold">{{ $monthlyStats['female_count'] ?? 0 }}</span></div>
                    <div class="flex justify-between pt-1"><span>Overall Attendance Rate:</span><span class="font-bold {{ ($monthlyStats['overall_avg_attendance'] ?? 0) >= 90 ? 'text-emerald-600' : 'text-amber-600' }}">{{ number_format($monthlyStats['overall_avg_attendance'] ?? 0, 1) }}%</span></div>
                </div>
            </div>
            <div class="summary-box bg-gray-50">
