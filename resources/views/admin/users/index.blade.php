        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $totalUsers = $users->total() ?? 0 }}</p>
                </div>
            ...
            <div class="flex items-center gap-2 text-sm">
                <span class="{{ $growthColor }} font-semibold flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-lg">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $activeUsers = $users->where('status', 'active')->count() ?? 0 }}</p>
                </div>
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between text-sm">
                    <span class="text-slate-500">{{ $activePercent }}% of total</span>
                    <span class="text-emerald-600 font-semibold">{{ $activeUsers }} active</span>
                </div>
                <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                    <div class="bg-gradient-to-r from-emerald-500 to-emerald-400 h-full rounded-full transition-all duration-500" style="width: {{ $activePercent }}%"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $inactiveUsers = $users->where('status', 'inactive')->count() ?? 0 }}</p>
                </div>
            ...
            <div class="space-y-2">
                <div class="flex items-center justify-between text-sm">
                    <span class="text-slate-500">{{ $inactivePercent }}% of total</span>
                    <span class="text-slate-600 font-semibold">{{ $inactiveUsers }} inactive</span>
                </div>
                <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                    <div class="bg-gradient-to-r from-slate-500 to-slate-400 h-full rounded-full transition-all duration-500" style="width: {{ $inactivePercent }}%"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-3xl font-bold text-slate-900">{{ $newThisWeek = $users->where('created_at', '>=', now()->subWeek())->count() ?? 0 }}</p>
                </div>
            ...
            @endphp
            <div class="flex items-center gap-2 text-sm">
                <span class="{{ $weekColor }} font-semibold flex items-center gap-1 bg-purple-50 px-2 py-1 rounded-lg">
                    <i class="fa-solid {{ $weekIcon }} text-xs"></i>
                    {{ abs($weekGrowth) }}%
                </span>
...
<div class="glass-card p-4 mb-8 animate-fade-in-up animate-delay-3">
    <div class="flex items-center gap-6">
        <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-slate-600">Active: <strong class="text-slate-900">{{ $activeUsers }}</strong></span>
        </div>
        <div class="flex items-center gap-3">
            <div class="w-3 h-3 rounded-full bg-slate-500"></div>
            <span class="text-sm text-slate-600">Inactive: <strong class="text-slate-900">{{ $inactiveUsers }}</strong></span>
        </div>
        <div class="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden flex">
            <div class="bg-emerald-500 h-full transition-all duration-500" style="width: {{ $activePercent }}%" title="Active {{ $activePercent }}%"></div>
            <div class="bg-slate-500 h-full transition-all duration-500" style="width: {{ $inactivePercent }}%" title="Inactive {{ $inactivePercent }}%"></div>
        </div>
        <span class="text-sm font-semibold text-slate-700">{{ $totalUsers }} Total</span>
    </div>
</div>
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($users as $user)
                              <tr class="table-row group" data-name="{{ strtolower($user->first_name . ' ' . $user->last_name . ' ' . $user->email) }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-slate-500">{{ $user->username ?? '@user' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
    ...
    @endphp
    <span class="status-badge inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border {{ $statusClass }}">
