...
{{-- Instant Action Modal (Success/Error) --}}
        ...
        @else
            <div class="divide-y divide-slate-100">
                @foreach($events as $event)
                    <div id="event-row-{{ $event->id }}" class="p-5 hover:bg-slate-50 transition-colors {{ $event->date->isToday() ? 'bg-amber-50/30' : '' }}">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                ...
                                <button type="button" onclick="openDeleteModal('{{ $event->id }}', '{{ $event->title }}')" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
