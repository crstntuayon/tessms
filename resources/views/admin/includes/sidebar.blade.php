...
<aside id="sidebar" class="sidebar w-72 bg-white border-r border-slate-200 h-screen flex flex-col">
    ...
    <span class="ml-auto bg-emerald-100 text-emerald-700 text-xs px-2.5 py-1 rounded-full font-bold">{{ $sidebarSectionCount }}</span>
        ...
        <a href="{{ route('admin.grades.index') }}" class="nav-item {{ request()->routeIs('admin.grades.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 text-slate-600 hover:text-slate-900 hover:bg-slate-50 rounded-xl font-medium transition-all group">
            <div class="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-amber-50 flex items-center justify-center transition-colors">
