    ...
    <div class="flex-1 lg:ml-72 transition-all duration-300">
        ...
        <main class="p-6 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8 animate-fade-in">
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover-lift">
                    <div class="flex items-center justify-between mb-4">
                        <div class="p-3 bg-blue-50 rounded-xl">
                            <i class="fas fa-user-graduate text-blue-600 text-xl"></i>
                        </div>
                        @php
    $maleCount = $section->students()->where('gender', 'male')->count();
    $femaleCount = $section->students()->where('gender', 'female')->count();
@endphp
                        <div class="text-right">
                            <span class="text-xs text-slate-500 block">M: {{ $maleCount }}</span>
                            <span class="text-xs text-slate-500 block">F: {{ $femaleCount }}</span>
                        </div>
                    </div>
                    <p class="text-2xl font-bold text-slate-900">{{ $students->total() }}</p>
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover-lift">
                    ...
                    @php
                        ...
                        $percentage = $students->total() > 0 ? round(($activeCount / $students->total()) * 100) : 0;
                    @endphp
                    <p class="text-2xl font-bold text-slate-900">{{ $activeCount }}</p>
                    <p class="text-sm text-slate-500">Active Today ({{ $percentage }}%)</p>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover-lift">
                    ...
                    @php
                        $atRiskCount = $students->filter(function($s) {
                            return isset($s->academic_status) && $s->academic_status == 'at_risk';
                        })->count();
                    @endphp
                    <p class="text-2xl font-bold text-slate-900">{{ $atRiskCount }}</p>
                    <p class="text-sm text-slate-500">Students at Risk</p>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover-lift">
                    ...
                    @php
                        $todayAttendance = $section->attendances()->whereDate('date', today())->count();
                        $attendanceRate = $students->total() > 0 ? round(($todayAttendance / $students->total()) * 100) : 0;
                    @endphp
...
<td class="px-6 py-4">
    ...
    <div class="flex items-center justify-end gap-1">
        ...
        <a href="{{ route('teacher.students.edit', $student->id) }}" 
           class="p-2 hover:bg-amber-50 text-slate-400 hover:text-amber-600 rounded-lg transition-colors" 
           title="Edit">
        ...
        <div class="relative inline-block" x-data="{ open: false }">
            <button @click="open = !open" 
                    class="p-2 hover:bg-slate-100 text-slate-400 hover:text-slate-600 rounded-lg transition-colors">
