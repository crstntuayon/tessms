        ...
        <div class="main-content">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8 animate-fade-in">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('teacher.books.createInventory') }}" class="btn-primary">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div class="glass-card p-6 animate-fade-in stagger-1">
                    ...
                    <form action="{{ route('teacher.books.inventory') }}" method="GET">
                    ...
                    @if($selectedSection)
                        <p class="text-xs text-slate-500 mt-2">
                            ...
                            Grade: {{ $selectedSection->gradeLevel->name ?? 'N/A' }}
                        </p>
                    @endif
                </div>
                ...
                <div class="stat-card animate-fade-in stagger-1">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-2xl font-bold text-slate-900">{{ $totalStats['total_titles'] }}</span>
                    </div>
                ...
                <div class="stat-card animate-fade-in stagger-2">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-2xl font-bold text-slate-900">{{ $totalStats['available'] }}</span>
                    </div>
                ...
                <div class="stat-card animate-fade-in stagger-3">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-2xl font-bold text-slate-900">{{ $totalStats['issued'] }}</span>
                    </div>
            ...
            <div class="glass-card p-6 mb-8 animate-fade-in stagger-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="text-center p-4 bg-slate-50 rounded-xl">
                        <p class="text-2xl font-bold text-slate-900">{{ $totalStats['total_copies'] }}</p>
                        <p class="text-xs text-slate-500 uppercase tracking-wider">Total Copies</p>
                    </div>
                    <div class="text-center p-4 bg-emerald-50 rounded-xl">
                        <p class="text-2xl font-bold text-emerald-600">{{ $totalStats['available'] }}</p>
                        <p class="text-xs text-emerald-600 uppercase tracking-wider">Available</p>
                    </div>
                    <div class="text-center p-4 bg-amber-50 rounded-xl">
                        <p class="text-2xl font-bold text-amber-600">{{ $totalStats['issued'] }}</p>
                        <p class="text-xs text-amber-600 uppercase tracking-wider">Issued</p>
                    </div>
                    <div class="text-center p-4 bg-red-50 rounded-xl">
                        <p class="text-2xl font-bold text-red-600">{{ $totalStats['damaged'] + $totalStats['lost'] }}</p>
            ...
            <div class="glass-card overflow-hidden animate-fade-in stagger-3">
                ...
                <div class="overflow-x-auto">
                    <table class="modern-table w-full" id="inventoryTable">
                        ...
                        <tbody>
                            @forelse($bookInventories as $inventory)
                                @php
                                ...
                                <tr>
                                    <td>
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-bold text-sm">
                                                {{ strtoupper(substr($inventory->title, 0, 2)) }}
                                            </div>
                                            <div>
                                                <p class="font-semibold text-slate-900 text-sm">{{ Str::limit($inventory->title, 40) }}</p>
                                                <p class="text-xs text-slate-500">
                                                    Code: {{ $inventory->book_code }}
                                                    @if($inventory->isbn) | ISBN: {{ $inventory->isbn }} @endif
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="subject-badge">
                                            {{ $inventory->subject_area }}
                                        </span>
                                        <p class="text-xs text-slate-400 mt-1">{{ $inventory->grade_level }}</p>
                                    </td>
                                    <td>
                                        <div class="w-24">
                                            <div class="flex justify-between text-xs mb-1">
                                                <span class="font-semibold text-slate-700">{{ $inventory->available_copies }}/{{ $inventory->total_copies }}</span>
                                            </div>
                                            <div class="progress-bar">
                                                <div class="progress-fill {{ $availabilityPercent < 20 ? 'bg-red-500' : 'bg-emerald-500' }}" 
                                    ...
                                    <td>
                                        <p class="text-sm font-semibold text-slate-700">
                                            {{ $inventory->issued_count ?? $inventory->issued_copies }}
                                        </p>
                                        <p class="text-xs text-slate-400">students</p>
                                    </td>
                                    <td>
                                        <div class="flex gap-1">
                                            <a href="{{ route('teacher.books.editInventory', $inventory) }}" 
                                               class="action-btn edit" title="Edit">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('teacher.books.destroyInventory', $inventory) }}" 
                                                  method="POST" 
                ...
                @if($bookInventories->hasPages())
                    <div class="p-6 border-t border-slate-100">
                        {{ $bookInventories->links() }}
                    </div>
                @endif
            </div>
...
<!-- Floating Add Button -->
<a href="{{ route('teacher.books.createInventory') }}" 
