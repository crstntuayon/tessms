...
<head>
    ...
    <title>Return Book - {{ $section->name }}</title>
    ...
    <div class="lg:ml-72 w-full min-h-screen p-8">
        ...
        @if($students->isEmpty())
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-lg p-12 text-center">
                ...
                <a href="{{ route('teacher.books.issue', $section) }}" class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium shadow-lg inline-flex items-center gap-2">
        ...
        @else
            ...
            <div class="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/50 shadow-xl shadow-slate-200/50 p-8" 
                ...
                <div x-show="step === 3" x-transition>
                    ...
                    <form method="POST" action="{{ route('teacher.books.storeReturn') }}" @submit.prevent="validateAndSubmit">
                        @csrf
                ...
                <div class="mt-8 grid grid-cols-3 gap-4 pt-6 border-t border-slate-200">
                    ...
                    <div class="text-center p-4 bg-amber-50 rounded-xl">
                        <p class="text-2xl font-bold text-amber-600">{{ $students->sum(fn($s) => $s->books->count()) }}</p>
