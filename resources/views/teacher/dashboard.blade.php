...
<head>
    ...
    <title>Tugawe ES - Teacher Dashboard | {{ $teacher->user->name ?? 'Teacher' }}</title>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="hidden sm:block">
                            ...
                            <div class="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                              <span>S.Y. {{ $activeSchoolYear->name ?? now()->format('Y') . '-' . (now()->format('Y') + 1) }}</span>
                                <span class="w-1 h-1 rounded-full bg-slate-300"></span>
                         <span class="text-indigo-600 font-semibold">{{ $currentQuarter }} Quarter</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="hidden md:flex items-center gap-3 bg-slate-100 px-4 py-2 rounded-2xl border border-slate-200">
                            ...
                            <div class="flex flex-col">
                                ...
                                <span id="liveDate" class="text-[10px] text-slate-500 uppercase tracking-wider">{{ now()->format('M d, Y') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3 pl-4 border-l border-slate-200">
                            <div class="hidden lg:block text-right">
                                <p class="text-sm font-bold text-slate-800">{{ $teacher->user->first_name}} {{ $teacher->user->middle_name ?? ' ' }} {{ $teacher->user->last_name}}</p>
                        ...
                        <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                            ...
                            <div class="flex gap-4">
                                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 text-center min-w-[100px] border border-white/20">
                                    <div class="text-3xl font-bold">{{ $totalStudents ?? 0 }}</div>
                                    <div class="text-xs text-indigo-200 uppercase tracking-wider mt-1">Students</div>
                                </div>
                                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 text-center min-w-[100px] border border-white/20">
                                    <div class="text-3xl font-bold">{{ $todayAttendanceRate ?? 0 }}%</div>
                                    <div class="text-xs text-indigo-200 uppercase tracking-wider mt-1">Attendance</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="glass-card rounded-2xl p-5 border-l-4 border-indigo-500">
    <div class="flex justify-between items-start mb-3">
        <div>
            <p class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Total Pupils</p>
            <h3 class="text-2xl font-bold text-slate-800">{{ $totalStudents ?? 0 }}</h3>
        </div>
    ...
    <div class="flex items-center justify-between text-xs mb-2">
        <span class="flex items-center gap-1 text-emerald-600 font-medium">
            <i class="fas fa-mars"></i> {{ $maleStudents ?? 0 }} Male
        </span>
        <span class="flex items-center gap-1 text-rose-600 font-medium">
            <i class="fas fa-venus"></i> {{ $femaleStudents ?? 0 }} Female
        </span>
    </div>
    ...
    <div class="w-full h-3 bg-slate-200 rounded-full overflow-hidden flex">
        <div class="bg-emerald-500 h-full" style="width: {{ $malePercent }}%"></div>
        <div class="bg-rose-500 h-full" style="width: {{ $femalePercent }}%"></div>
    </div>
</div>
                        ...
                        <div class="glass-card rounded-2xl p-5 border-l-4 border-emerald-500">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    <p class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Today's Attendance</p>
                                    <h3 class="text-2xl font-bold text-slate-800">{{ $todayAttendanceRate ?? 0 }}%</h3>
                                </div>
                            ...
                            <div class="w-full bg-slate-200 rounded-full h-2 mb-2">
                                <div class="bg-emerald-500 h-2 rounded-full transition-all duration-700" style="width: {{ $todayAttendanceRate ?? 0 }}%"></div>
                            </div>
                        ...
                        <div class="glass-card rounded-2xl p-5 border-l-4 border-violet-500">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    <p class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Pending Grades</p>
                                    <h3 class="text-2xl font-bold text-slate-800">{{ $pendingGradesCount ?? 0 }}</h3>
                                </div>
                            ...
                            @if(($overdueGrading ?? 0) > 0)
                                <span class="text-rose-600 text-xs font-medium flex items-center gap-1 bg-rose-50 px-2 py-1 rounded-lg w-fit">
                                    <i class="fas fa-exclamation-circle"></i> {{ $overdueGrading }} Overdue
                                </span>
                            @else
                                <span class="text-emerald-600 text-xs font-medium flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded-lg w-fit">
                        ...
                        <div class="glass-card rounded-2xl p-5 border-l-4 border-rose-500">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    <p class="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">Students At Risk</p>
                                    <h3 class="text-2xl font-bold text-slate-800">{{ $atRiskCount ?? 0 }}</h3>
                                </div>
                            ...
                            <div class="flex items-center gap-2 text-xs">
                                ...
                                <span class="text-amber-600 font-medium bg-amber-50 px-2 py-1 rounded-lg">{{ $chronicAbsentees ?? 0 }} Chronic Absentee</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="glass-card rounded-2xl p-6">
                                ...
                                <form action="{{ route('teacher.dashboard') }}" method="GET" id="sectionForm">
                                    <select name="section_id" onchange="document.getElementById('sectionForm').submit()" 
                                            class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white text-sm font-medium text-slate-700 cursor-pointer">
                                        @foreach($sections ?? [] as $section)
                                            <option value="{{ $section->id }}" {{ ($activeSection->id ?? '') == $section->id ? 'selected' : '' }}>
                                                Grade {{ $section->gradeLevel->name ?? 'N/A' }} - {{ $section->name }} ({{ $section->students->count() }} students)
                                            </option>
                                        @endforeach
                                    </select>
                                </form>
                                ...
                                <div class="mt-4 space-y-3">
                                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                        ...
                                        <span class="text-xs font-bold text-slate-800">{{ $activeSection->teacher->full_name ?? 'N/A' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                        <span class="text-xs font-medium text-slate-600">Room</span>
                                        <span class="text-xs font-bold text-slate-800">{{ $activeSection->room_number ?? 'TBA' }}</span>
                                    </div>
                                    <div class="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                                        <span class="text-xs font-medium text-slate-600">School Year</span>
                                        <span class="text-xs font-bold text-slate-800">{{ $schoolYear ?? now()->format('Y') . '-' . (now()->format('Y') + 1) }}</span>
                                    </div>
                                </div>
                                @endif
                            </div>
                            ...
                            <!-- Mini Calendar -->
<div class="glass-card rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300">
    ...
    <div class="flex justify-between items-center mb-5">
        ...
        <span class="text-xs text-indigo-600 font-semibold bg-indigo-50 px-3 py-1 rounded-full">
            {{ now()->format('F Y') }}
        </span>
    </div>

    <!-- Days -->
    <div class="grid grid-cols-7 gap-2 text-center text-[10px] mb-3 text-slate-400 font-semibold uppercase">
        <span>S</span><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span>
    </div>

    <!-- Calendar Grid -->
    <div class="grid grid-cols-7 gap-2" id="miniCalendar">
        <!-- JS GENERATED -->
    </div>

    <!-- Legend -->
    <div class="mt-5 flex justify-center gap-4 text-[11px] bg-slate-50 p-3 rounded-xl border border-slate-100">
                            ...
                            <div class="glass-card rounded-2xl p-6 bg-gradient-to-br from-indigo-600 to-purple-700 text-white">
                                ...
                                <div class="space-y-4">
                                    <div class="flex justify-between items-center">
                                        <span class="text-sm opacity-80">Present Today</span>
                                        <span class="text-lg font-bold">{{ $todayStats['present'] ?? 0 }}</span>
                                    </div>
                                    <div class="w-full bg-white/20 rounded-full h-1">
                                        <div class="bg-emerald-400 h-1 rounded-full" style="width: {{ (($todayStats['present'] ?? 0) / max($totalStudents, 1)) * 100 }}%"></div>
                                    </div>
                                    
                                    <div class="flex justify-between items-center pt-2">
                                        <span class="text-sm opacity-80">Absent Today</span>
                                        <span class="text-lg font-bold">{{ $todayStats['absent'] ?? 0 }}</span>
                                    </div>
                                    <div class="w-full bg-white/20 rounded-full h-1">
                                        <div class="bg-rose-400 h-1 rounded-full" style="width: {{ (($todayStats['absent'] ?? 0) / max($totalStudents, 1)) * 100 }}%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="glass-card rounded-2xl p-6">
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                            <div>
                                ...
                                <p class="text-sm text-slate-500 mt-1">{{ $currentQuarter ?? '1st' }} Quarter • Real-time grade analysis</p>
                            </div>
                            <div class="flex gap-2">
                                ...
                                <a href="{{ route('teacher.exports.sf9', ['section_id' => $activeSection->id ?? '']) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 text-sm font-medium shadow-lg shadow-indigo-500/30 transition-all flex items-center gap-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6" id="subjectCards">
                            ...
                            <div class="subject-card border border-slate-200 rounded-xl p-4 hover:shadow-lg transition-all hover:border-indigo-300 cursor-pointer bg-white" data-subject-id="{{ $stats['subject_id'] }}">
                                <div class="flex items-center gap-3 mb-3">
                                    <div class="w-10 h-10 bg-{{ $stats['color'] ?? 'indigo' }}-100 rounded-lg flex items-center justify-center text-{{ $stats['color'] ?? 'indigo' }}-600">
                                        <i class="fas {{ $stats['icon'] ?? 'fa-book' }}"></i>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <h4 class="font-bold text-slate-800 text-sm truncate">{{ $stats['name'] }}</h4>
                                        <p class="text-[10px] text-slate-500">WW:{{ $stats['ww_weight'] ?? 40 }}% PT:{{ $stats['pt_weight'] ?? 40 }}%</p>
                                    </div>
                                </div>
                                <div class="flex justify-between items-end mb-2">
                                    ...
                                    <div class="text-right">
                                        ...
                                        @endphp
                                        <span class="{{ $class }} px-2 py-1 rounded-lg text-[10px] font-bold">{{ $label }}</span>
                                    </div>
                                </div>
                                <div class="w-full bg-slate-100 rounded-full h-1.5 mb-2">
                                    <div class="bg-{{ $stats['color'] ?? 'indigo' }}-500 h-1.5 rounded-full transition-all duration-700" style="width: {{ min($avg, 100) }}%"></div>
                                </div>
                                ...
                                <table class="w-full text-sm">
                                    <thead class="bg-slate-50">
                                        <tr>
                                            ...
                                            @foreach($subjects ?? [] as $subject)
                                                <th class="px-4 py-3 text-center text-xs font-bold text-slate-600 uppercase">{{ $subject->code }}</th>
                                            @endforeach
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="lg:col-span-2 glass-card rounded-2xl p-6">
                            <div class="flex justify-between items-center mb-6">
                                ...
                                <a href="{{ route('teacher.grades.index') }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">View All</a>
                            </div>
                        ...
                        <div class="space-y-6">
                            <div class="glass-card rounded-2xl p-6">
                                ...
                                <a href="{{ route('teacher.exports.sf1') }}?section_id={{ $activeSection->id ?? '' }}"
   class="p-4 bg-emerald-50 hover:bg-emerald-100 rounded-xl text-center transition-all group border border-transparent hover:border-emerald-200">
    ...
    <p class="text-xs font-bold text-emerald-800">Export SF1</p>
                                    ...
                                    <a href="{{ route('teacher.reports.index') }}" class="p-4 bg-violet-50 hover:bg-violet-100 rounded-xl text-center transition-all group border border-transparent hover:border-violet-200">
                            ...
                            <div class="glass-card rounded-2xl p-6 bg-gradient-to-br from-slate-800 to-slate-900 text-white">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <span class="bg-white/20 px-2 py-1 rounded text-xs">{{ $upcomingDeadlines->count() ?? 0 }}</span>
                                </div>
                                <ul class="space-y-3 text-sm">
                                    @forelse($upcomingDeadlines ?? [] as $deadline)
                                    <li class="flex items-start gap-3 bg-white/10 rounded-xl p-3 backdrop-blur-sm border border-white/10">
                                        <i class="fas fa-circle text-[8px] mt-1.5 {{ $deadline->is_urgent ? 'text-rose-400' : 'text-amber-400' }}"></i>
                                        <div>
                                            <span class="font-medium block text-sm">{{ $deadline->title }}</span>
                                            <p class="text-xs text-slate-400 mt-0.5">Due {{ $deadline->due_date->diffForHumans() }}</p>
                                        </div>
                                    </li>
                                    @empty
                                    <li class="text-slate-400 text-center py-4 bg-white/5 rounded-xl border border-white/10">
                                        <i class="fas fa-check-circle mb-2 block text-xl"></i>
...
function printClassRecord() {
    ...
    window.location.href = `{{ route('teacher.reports.class-record') }}?section_id={{ $activeSection->id ?? '' }}`;
}
        // Toast Notifications
