...
<div class="flex h-screen">
    ...
    <div class="flex-1 h-screen flex flex-col bg-slate-50 overflow-hidden transition-all duration-300"
        ...
        {{-- Stats Bar --}}
        <div class="px-6 py-4 bg-white border-b border-slate-100 shrink-0">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-blue-50 rounded-xl p-4 border border-blue-100">
                    <p class="text-xs font-semibold text-blue-600 uppercase tracking-wider">Total Events</p>
                    <p class="text-2xl font-bold text-blue-900 mt-1">{{ $events->count() }}</p>
                </div>
                <div class="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
                    <p class="text-xs font-semibold text-emerald-600 uppercase tracking-wider">Upcoming</p>
                    <p class="text-2xl font-bold text-emerald-900 mt-1">{{ $events->where('date', '>=', now())->count() }}</p>
                </div>
                ...
                <div class="bg-rose-50 rounded-xl p-4 border border-rose-100">
                    ...
                    <p class="text-2xl font-bold text-rose-900 mt-1">{{ $events->where('date', '<', now())->count() }}</p>
                </div>
            </div>
        </div>
            ...
            @else
                <div class="space-y-4 max-w-4xl mx-auto">
                    @foreach($events as $event)
                        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow {{ $event->date->isToday() ? 'ring-1 ring-amber-200' : '' }}">
                            <div class="p-5">
                                <div class="flex items-start gap-4">
                                    {{-- Date Box --}}
                                    <div class="w-14 h-14 rounded-xl flex flex-col items-center justify-center shrink-0 {{ $event->date->isPast() ? 'bg-slate-100 text-slate-500' : ($event->date->isToday() ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600') }}">
                                        <span class="text-[10px] font-bold uppercase">{{ $event->date->format('M') }}</span>
                                    ...
                                    <div class="flex items-center gap-1 shrink-0">
                                        <a href="{{ route('teacher.events.show', $event) }}" class="px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors">
                                            View Details
                                        </a>
                                    </div>
                                </div>
