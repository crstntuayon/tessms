...
@section('title', 'School Days Configuration')
...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-500 mt-1">{{ $section->name }} • {{ $section->gradeLevel->name ?? 'N/A' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-4">
        ...
        <div class="flex-1">
            <h3 class="font-bold text-amber-900 text-lg">Section Finalized</h3>
            <p class="text-amber-700">This section has been finalized and is locked. Contact the administrator if you need to make changes.</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center p-4 bg-blue-50 rounded-xl">
                ...
                <p class="text-2xl font-bold text-blue-600">{{ $schoolDays->getMonthName() }}</p>
                <p class="text-sm text-slate-500">{{ $year }}</p>
            </div>
            <div class="text-center p-4 bg-emerald-50 rounded-xl">
                <p class="text-sm text-slate-600 mb-1">School Days</p>
                <p class="text-2xl font-bold text-emerald-600">{{ $schoolDays->total_school_days }}</p>
                <p class="text-sm text-slate-500">Total</p>
            </div>
            <div class="text-center p-4 bg-rose-50 rounded-xl">
                <p class="text-sm text-slate-600 mb-1">Non-School Days</p>
                <p class="text-2xl font-bold text-rose-600">{{ $schoolDays->getNonSchoolDaysCount() }}</p>
                <p class="text-sm text-slate-500">Holidays/Suspensions</p>
            </div>
            <div class="text-center p-4 bg-purple-50 rounded-xl">
                <p class="text-sm text-slate-600 mb-1">Status</p>
                @if($schoolDays->is_finalized)
                <p class="text-lg font-bold text-purple-600"><i class="fas fa-lock mr-1"></i> Finalized</p>
                @else
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-7 gap-2">
                ...
                <div class="aspect-square rounded-xl border-2 {{ $day['is_school_day'] ? 'border-emerald-200 bg-emerald-50' : ($day['is_weekend'] ? 'border-slate-200 bg-slate-100' : 'border-rose-200 bg-rose-50') }} p-2 relative group">
                    <span class="text-sm font-semibold {{ $day['is_school_day'] ? 'text-emerald-700' : ($day['is_weekend'] ? 'text-slate-500' : 'text-rose-700') }}">
                        {{ $day['day'] }}
                    </span>
                    
                    @if($day['is_non_school_day'] && $day['reason'])
                    <div class="mt-1 text-[10px] text-rose-600 leading-tight line-clamp-2" title="{{ $day['reason'] }}">
                        {{ $day['reason'] }}
                    </div>
                    @endif

                    @if(!$day['is_weekend'] && $isEditable)
                    <button type="button" 
                            onclick="toggleSchoolDay('{{ $day['date'] }}', {{ $day['is_non_school_day'] ? 'true' : 'false' }})"
                        ...
                        <i class="fas {{ $day['is_non_school_day'] ? 'fa-plus text-emerald-600' : 'fa-minus text-rose-600' }} text-lg"></i>
                    </button>
                    @endif
                </div>
                @endforeach
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            @if(count($schoolDays->non_school_days ?? []) > 0)
            <div class="space-y-3 max-h-96 overflow-y-auto">
                @foreach($schoolDays->non_school_days as $nonSchoolDay)
                <div class="flex items-center gap-3 p-3 bg-rose-50 rounded-xl border border-rose-100">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-900">
                            {{ \Carbon\Carbon::parse($nonSchoolDay['date'])->format('M d, Y') }}
                        </p>
                        <p class="text-xs text-slate-500 truncate">{{ $nonSchoolDay['reason'] }}</p>
                    </div>
                    @if($isEditable)
                    <button type="button" 
                            onclick="removeNonSchoolDay('{{ $nonSchoolDay['date'] }}')"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mt-6">
        ...
        <form action="{{ route('teacher.sections.attendance.school-days.update', $section) }}" method="POST">
            @csrf
            ...
            <textarea name="teacher_notes" rows="4" 
                      {{ !$isEditable ? 'disabled' : '' }}
                      class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 {{ !$isEditable ? 'bg-slate-100' : '' }}"
                      placeholder="Add notes about holidays, suspensions, or other important information...">{{ $schoolDays->teacher_notes }}</textarea>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mt-6">
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('teacher.sections.attendance.finalize', $section) }}" method="POST">
                @csrf
                <button type="submit" 
                        ...
                        onclick="return confirm('Are you sure you want to finalize attendance for this section? This action cannot be undone.')">
                    <i class="fas fa-lock mr-2"></i>Finalize Attendance
                </button>
            </form>
        </div>
    </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="date" id="nonSchoolDate" required
                           min="{{ $year }}-{{ str_pad($month, 2, '0', STR_PAD_LEFT) }}-01"
                           max="{{ $year }}-{{ str_pad($month, 2, '0', STR_PAD_LEFT) }}-{{ cal_days_in_month(CAL_GREGORIAN, $month, $year) }}"
...
function addNonSchoolDay(event) {
    ...
    fetch('{{ route("teacher.sections.attendance.non-school-day.add", $section) }}', {
        method: 'POST',
        headers: {
...
function removeNonSchoolDay(date) {
    ...
    fetch('{{ route("teacher.sections.attendance.non-school-day.remove", $section) }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
