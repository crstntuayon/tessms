...
<head>
    ...
    <title>Take Attendance - {{ $section->name }}</title>
...
<body class="antialiased">
    <div x-data="mobileAttendance({{ $section->id }}, {{ json_encode($students) }})" 
         x-init="init()"
        ...
        <div class="sticky top-0 z-30 bg-white border-b border-slate-200 safe-top">
            <div class="px-4 py-3">
                ...
                <div class="flex items-center justify-between mb-3">
                    ...
                    <div class="text-right min-w-[80px]">
                        ...
                        <p class="text-xs text-slate-500 truncate max-w-[100px]">{{ $section->name }}</p>
                    </div>
                </div>
            ...
            <div class="px-4 py-3 border-t border-slate-200">
                @include('components.location-verifier')
            </div>
        </div>
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 safe-bottom z-40">
            <div class="max-w-lg mx-auto flex space-x-3">
                <button @click="save()" 
                    ...
                    <i x-show="saving" class="fas fa-spinner fa-spin text-lg"></i>
        ...
        function mobileAttendance(sectionId, initialStudents) {
            return {
                ...
                async save() {
                    ...
                    try {
                        if (this.isOnline) {
                            const response = await fetch('{{ route('teacher.attendance.bulk-store') }}', {
                                method: 'POST',
                                headers: {
