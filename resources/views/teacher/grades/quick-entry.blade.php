...
@section('title', 'Quick Grade Entry')
...
<div class="max-w-7xl mx-auto" x-data="{ 
    ...
    hasChanges: false,
    showSubmitAll: false,
    grades: {{ json_encode($grades->mapWithKeys(function($g) {
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-500">{{ $section->name }} • Quarter {{ $currentQuarter }}</p>
        </div>
    ...
    <!-- Grade Form -->
    <form id="gradeForm" action="{{ route('teacher.grades.quick-save', $section) }}" method="POST" 
                    ...
                    <thead class="bg-slate-50 sticky top-0">
                        <tr>
                            ...
                            @foreach($subjects as $subject)
                                <th class="px-3 py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider w-24" title="{{ $subject->name }}">
                                    <div class="truncate max-w-[80px]">{{ Str::limit($subject->code ?? $subject->name, 8) }}</div>
                                </th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @foreach($students as $index => $student)
                            <tr class="hover:bg-slate-50">
                                <td class="px-4 py-2 text-sm text-slate-400">{{ $index + 1 }}</td>
                                <td class="px-4 py-2">
                                ...
                                @foreach($subjects as $subject)
                                    ...
                                    <td class="px-2 py-2">
                                        <input type="number" 
                                               ...
                                               :class="{ 'bg-yellow-50 border-yellow-300': grades['{{ $gradeKey }}'] !== $el.value && $el.value !== '' }"
                                               x-on:input="grades['{{ $gradeKey }}'] = $el.value"
                                               tabindex="{{ ($index * count($subjects)) + $loop->parent->iteration }}">
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
