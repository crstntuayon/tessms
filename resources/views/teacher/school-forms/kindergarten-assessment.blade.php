...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="ml-72 p-6">
        ...
        <div class="mb-6 flex justify-between items-start">
            <h1 class="text-2xl font-bold text-slate-800 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-normal text-gray-500 mt-0">{{ $lang == 'cebuano' ? 'Pagtimbang-timbang sa Kindergarten' : 'Kindergarten Developmental Assessment' }}</p>
                </div>
            </h1>
            <a href="?{{ http_build_query(array_merge(request()->except('lang'), ['lang' => $lang == 'cebuano' ? 'english' : 'cebuano'])) }}" 
               class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
                {{ $lang == 'cebuano' ? 'Switch to English' : 'Switch to Cebuano' }}
            </a>
        </div>
            ...
            <div id="successErrorContent" class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 modal-pop">
                ...
                <div id="successState" class="hidden">
                    <div class="bg-emerald-50 rounded-t-2xl p-6 border-b border-emerald-100 text-center">
                        ...
                        <h3 class="text-xl font-bold text-emerald-900">{{ $lang == 'cebuano' ? 'Malampuson!' : 'Success!' }}</h3>
                    </div>
                    <div class="p-6 text-center">
                        <p class="text-slate-600 mb-4" id="successMessageText">{{ session('success') }}</p>
                ...
                <div id="errorState" class="hidden">
                    <div class="bg-red-50 rounded-t-2xl p-6 border-b border-red-100 text-center">
                        ...
                        <h3 class="text-xl font-bold text-red-900">{{ $lang == 'cebuano' ? 'Sayop!' : 'Error!' }}</h3>
                    </div>
                    <div class="p-6 text-center">
                        <p class="text-slate-600 mb-4" id="errorMessageText">{{ session('error') }}</p>
        ...
        <div class="mb-4 bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="font-medium text-amber-800">{{ $lang == 'cebuano' ? 'Mga Kulang nga Marka:' : 'Missing Ratings:' }}</span>
            </div>
        ...
        @if($selectedSection && $finalization)
            @if($finalization->grades_finalized || $finalization->is_locked)
            <div class="mb-4 bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-bold text-emerald-800">
                            {{ $lang == 'cebuano' ? 'Na-finalize na ang mga marka' : 'Assessments Finalized' }}
                        </p>
                ...
                <div class="text-right text-sm text-emerald-600">
                    <p>{{ $lang == 'cebuano' ? 'Gi-finalize ni:' : 'Finalized by:' }}</p>
                    <p class="font-medium">{{ $finalization->finalizedByUser?->name ?? 'N/A' }}</p>
                    <p>{{ $finalization->grades_finalized_at->format('M d, Y h:i A') }}</p>
                </div>
                @endif
            </div>
            @endif
        ...
        <div class="bg-white rounded-lg p-6 shadow-sm mb-6">
            <form method="GET" action="{{ route('teacher.kindergarten.assessment') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select name="section_id" onchange="this.form.submit()" class="w-full px-4 py-2 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                        @foreach($kinderSections as $section)
            ...
            <div class="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between">
                ...
                <button type="button" onclick="showFinalizeKindergartenModal()" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium flex items-center gap-2">
                    ...
                    {{ $lang == 'cebuano' ? 'I-finalize ang mga Marka' : 'Finalize Assessments' }}
                </button>
                @endif
            </div>
            @endif
        ...
        <div class="bg-white rounded-lg p-4 shadow-sm mb-6 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-xl font-bold">
                    {{ substr($selectedStudent->user->first_name ?? 'S', 0, 1) }}
                </div>
                <div>
                    ...
                    <p class="text-sm text-slate-500">
                        ...
                        Quarter: <span class="font-medium text-blue-600">{{ $selectedQuarter }}</span>
                    </p>
                </div>
            </div>
            <div class="flex gap-4 text-sm">
                @php $ratingScale = config('kindergarten.rating_scale'); @endphp
                <div class="rating-legend"><span class="font-bold text-blue-700">B</span> = {{ $ratingScale['B']['label'][$lang] }}</div>
                <div class="rating-legend"><span class="font-bold text-blue-700">D</span> = {{ $ratingScale['D']['label'][$lang] }}</div>
                <div class="rating-legend"><span class="font-bold text-blue-700">C</span> = {{ $ratingScale['C']['label'][$lang] }}</div>
            </div>
        </div>

        <!-- Assessment Form -->
        <form method="POST" action="{{ route('teacher.kindergarten.store') }}">
            @csrf
                        ...
                        <div class="indicator-row">
                            ...
                            <div class="rating-options">
                                <div class="rating-option">
                                    <input type="radio" name="ratings[{{ $domainKey }}][{{ $indicatorKey }}]" id="{{ $domainKey }}_{{ $indicatorKey }}_B" value="B" {{ $existingRating == 'B' ? 'checked' : '' }} {{ !$isEditable ? 'disabled' : '' }}>
                                    <label for="{{ $domainKey }}_{{ $indicatorKey }}_B">B</label>
                                </div>
                                <div class="rating-option">
                                    <input type="radio" name="ratings[{{ $domainKey }}][{{ $indicatorKey }}]" id="{{ $domainKey }}_{{ $indicatorKey }}_D" value="D" {{ $existingRating == 'D' ? 'checked' : '' }} {{ !$isEditable ? 'disabled' : '' }}>
                                    <label for="{{ $domainKey }}_{{ $indicatorKey }}_D">D</label>
                                </div>
                                <div class="rating-option">
                                    <input type="radio" name="ratings[{{ $domainKey }}][{{ $indicatorKey }}]" id="{{ $domainKey }}_{{ $indicatorKey }}_C" value="C" {{ $existingRating == 'C' ? 'checked' : '' }} {{ !$isEditable ? 'disabled' : '' }}>
                                    <label for="{{ $domainKey }}_{{ $indicatorKey }}_C">C</label>
                                </div>
                            </div>
                        </div>
                        @endforeach
            ...
            <div class="flex justify-between items-center mt-6 mb-12">
                <div class="text-sm text-slate-500">
                    ...
                    @if($isEditable)
                        {{ $lang == 'cebuano' ? 'All ratings are automatically saved when you click Save Assessment' : 'All ratings are automatically saved when you click Save' }}
                    @else
                        {{ $lang == 'cebuano' ? 'Ang mga marka nalock na. Dili na makausab.' : 'Assessments are locked. No further changes allowed.' }}
                    @endif
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('teacher.sf9') }}?student_id={{ $selectedStudent->id }}&lang={{ $lang }}" target="_blank" class="px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-700 font-medium flex items-center gap-2">
                        <i class="fas fa-eye"></i>{{ $lang == 'cebuano' ? 'View SF9' : 'View Report Card' }}
                    </a>
                    @if($isEditable)
                        <button type="submit" class="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium flex items-center gap-2">
                            <i class="fas fa-save"></i>{{ $lang == 'cebuano' ? 'Save Assessment' : 'Save Ratings' }}
                        </button>
                    @else
                        <button type="button" disabled class="px-8 py-3 bg-gray-400 text-white rounded-lg font-medium flex items-center gap-2 cursor-not-allowed">
                            <i class="fas fa-lock"></i>{{ $lang == 'cebuano' ? 'Nalock Na' : 'Locked' }}
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full mx-4 transform transition-all" style="animation: modal-pop 0.3s ease-out;">
            <div class="bg-emerald-50 rounded-t-2xl p-6 border-b border-emerald-100">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-emerald-900">{{ $lang == 'cebuano' ? 'I-finalize ang mga Marka?' : 'Finalize Assessments?' }}</h3>
                        <p class="text-sm text-emerald-600">{{ $lang == 'cebuano' ? 'Dili na makausab human niini' : 'This action cannot be undone' }}</p>
                    </div>
                </div>
            </div>
            <div class="p-6">
                <div class="bg-slate-50 rounded-xl p-4 mb-4 space-y-3">
                    <p class="text-sm text-slate-700">
                        ...
                        {{ $lang == 'cebuano' ? 'I-finalize nimo ang mga marka para sa' : 'You are about to finalize assessments for' }} <strong>{{ $selectedSection->name }}</strong>.
                    </p>
                    <div class="text-sm text-slate-600 space-y-2 ml-6">
                        <p><i class="fas fa-check text-emerald-500 mr-2"></i>{{ $lang == 'cebuano' ? 'Ang tanang marka nalock na' : 'All ratings will be locked' }}</p>
                        <p><i class="fas fa-check text-emerald-500 mr-2"></i>{{ $lang == 'cebuano' ? 'Ang 8 ka developmental domains para sa Q1-Q4 gi-finalize na' : 'All 8 developmental domains for Q1-Q4 will be finalized' }}</p>
                        <p><i class="fas fa-check text-emerald-500 mr-2"></i>{{ $lang == 'cebuano' ? 'Kinahanglan ang tabang sa admin kung usbon' : 'Admin assistance required for any changes' }}</p>
                    </div>
                </div>
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
                    <p class="text-sm text-amber-800">
                        <i class="fas fa-exclamation-circle mr-2"></i>
                        <strong>{{ $lang == 'cebuano' ? 'Importanteng butang:' : 'Important:' }}</strong> {{ $lang == 'cebuano' ? 'Siguradoha nga na-complete na ang tanang 8 ka domains para sa tanang estudyante sa Q1-Q4.' : 'Make sure you have rated ALL 8 developmental domains for ALL students across ALL quarters (Q1-Q4) before finalizing.' }}
                    </p>
                </div>
                <form id="finalizeKindergartenForm" action="{{ route('teacher.kindergarten.finalize', $selectedSection) }}" method="POST">
                    @csrf
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button type="button" 
                            ...
                            {{ $lang == 'cebuano' ? 'Kanselahon' : 'Cancel' }}
                        </button>
                        <button type="submit" 
                            ...
                            <i class="fas fa-lock mr-2"></i>{{ $lang == 'cebuano' ? 'Oo, I-finalize' : 'Yes, Finalize' }}
                        </button>
                    </div>
                </form>
            </div>
    ...
    <div id="finalizingModal" class="hidden fixed inset-0 z-[10000] flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-8 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-800">{{ $lang == 'cebuano' ? 'Nag-finalize...' : 'Finalizing...' }}</h3>
            <p class="text-sm text-slate-500 mt-1">{{ $lang == 'cebuano' ? 'Palihog hulat samtang gi-lock ang mga marka' : 'Please wait while we lock your assessment records' }}</p>
        </div>
    </div>
        ...
        <div id="finalizeResultContent" class="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 modal-pop">
            ...
            <div id="finalizeResultSuccess" class="hidden">
                <div class="bg-emerald-50 rounded-t-2xl p-6 border-b border-emerald-100 text-center">
                    ...
                    <h3 class="text-xl font-bold text-emerald-900">{{ $lang == 'cebuano' ? 'Malampuson nga Finalize!' : 'Finalized Successfully!' }}</h3>
                    <p class="text-sm text-emerald-600 mt-1">{{ $lang == 'cebuano' ? 'Ang mga marka nalock na' : 'Assessments have been locked' }}</p>
                </div>
                <div class="p-6 text-center">
                    <p class="text-slate-600 mb-4" id="finalizeSuccessMessage">{{ $lang == 'cebuano' ? 'Ang mga marka malampuson nga na-finalize.' : 'Kindergarten assessments have been finalized successfully.' }}</p>
            ...
            <div id="finalizeResultError" class="hidden">
                <div class="bg-red-50 rounded-t-2xl p-6 border-b border-red-100 text-center">
                    ...
                    <h3 class="text-xl font-bold text-red-900">{{ $lang == 'cebuano' ? 'Napakyas ang Finalize!' : 'Finalization Failed!' }}</h3>
                    <p class="text-sm text-red-600 mt-1">{{ $lang == 'cebuano' ? 'Dili ma-finalize ang mga marka' : 'Unable to finalize assessments' }}</p>
                </div>
                <div class="p-6 text-center">
                    <p class="text-slate-600 mb-4" id="finalizeErrorMessage">{{ $lang == 'cebuano' ? 'Naay sayup samtang gi-finalize ang mga marka.' : 'An error occurred while finalizing assessments.' }}</p>
    ...
    document.getElementById('finalizeKindergartenForm').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (response.status === 419) {
                ...
                showFinalizeResultModal('error', '{{ $lang == 'cebuano' ? 'Na-expire ang sesyon. Palihog refresh sa page ug sulayi pag-usab.' : 'Session expired. Please refresh the page and try again.' }}');
                return;
            }
            
            const data = await response.json().catch(() => ({
                success: response.ok,
                message: response.ok ? '{{ $lang == 'cebuano' ? 'Malampuson nga na-finalize!' : 'Finalized successfully!' }}' : '{{ $lang == 'cebuano' ? 'Napakyas ang pag-finalize' : 'Failed to finalize' }}'
            }));
            ...
            if (data.success) {
                showFinalizeResultModal('success', data.message || '{{ $lang == 'cebuano' ? 'Malampuson nga na-finalize ang mga marka!' : 'Kindergarten assessments finalized successfully!' }}');
                // Reload page after showing success
                setTimeout(() => {
                    window.location.reload();
                }, 2000);
            } else {
                showFinalizeResultModal('error', data.message || '{{ $lang == 'cebuano' ? 'Napakyas ang pag-finalize sa mga marka' : 'Failed to finalize assessments' }}');
            }
        } catch (error) {
            console.error('Finalize error:', error);
            document.getElementById('finalizingModal').classList.add('hidden');
            showFinalizeResultModal('error', '{{ $lang == 'cebuano' ? 'Network error. Palihog sulayi pag-usab.' : 'Network error. Please try again.' }}');
        }
    });
    ...
    @if(session('success'))
        document.addEventListener('DOMContentLoaded', function() {
            showSuccessErrorModal('success', '{{ session('success') }}');
        });
    @endif
    @if(session('error'))
        document.addEventListener('DOMContentLoaded', function() {
            showSuccessErrorModal('error', '{{ session('error') }}');
        });
    @endif
    </script>
    @endif
