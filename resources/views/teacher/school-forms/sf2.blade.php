...
<body class="bg-slate-100 min-h-screen" x-data="{ mobileOpen: false }" @keydown.escape.window="mobileOpen = false">
    ...
    <div class="lg:ml-72 p-6">
        ...
        <div class="no-print mb-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            <form method="GET" action="{{ route('teacher.sf2') }}" class="flex items-end gap-4">
                <div class="flex-1">
            ...
            <table class="sf2-table">
                <thead>
                    <tr>
                        ...
                        @foreach($displayDays as $index => $day)
                            <th class="date-header" style="width: 2.5%;">{{ $day }}</th>
                        @endforeach
                        @php
                    ...
                    <tr>
                        @foreach($displayDays as $index => $day)
                            ...
                            @endphp
                            <th class="day-header">{{ $dayLetter }}</th>
                        @endforeach
                        @for($i = 0; $i < $remainingCols; $i++)
                ...
                <tbody>
                    <!-- MALE Section -->
                    <tr>
                        <td colspan="{{ 29 + count($displayDays) }}" class="text-left font-bold bg-gray-100 text-[9px] uppercase pl-2">MALE</td>
                    </tr>
                    ...
                    @forelse($maleEnrollments as $index => $enrollment)
                        ...
                        <tr>
                            ...
                            @endfor
                            <td class="font-bold text-[9px]">{{ $absentCount > 0 ? $absentCount : '' }}</td>
                            <td class="font-bold text-[9px]">{{ $tardyCount > 0 ? $tardyCount : '' }}</td>
                            <td class="text-[8px]">{{ $student->attendance_remarks ?? '' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ 29 + count($displayDays) }}" class="text-center py-2 text-slate-400 text-[8px]">No male students</td>
                        </tr>
                    @endforelse

                    <!-- MALE TOTAL ROW -->
                    <tr class="bg-gray-50 font-bold">
                        <td colspan="{{ 2 + count($displayDays) + $remainingCols }}" class="text-right text-[9px] pr-2">MALE TOTAL PER DAY:</td>
                        @foreach($displayDays as $day)
                            @php
                                $dateStr = sprintf('%04d-%02d-%02d', $year, $monthNum, $day);
                            ...
                            @endphp
                            <td class="text-[8px]">{{ $malePresent }}</td>
                        @endforeach
                        @for($i = 0; $i < $remainingCols; $i++)
                            <td class="text-[8px]"></td>
                        @endfor
                        <td class="text-[9px]">{{ $maleTotalAbsent > 0 ? $maleTotalAbsent : '' }}</td>
                        <td class="text-[9px]">{{ $maleTotalTardy > 0 ? $maleTotalTardy : '' }}</td>
                        <td></td>
                    </tr>

                    <!-- FEMALE Section -->
                    <tr>
                        <td colspan="{{ 29 + count($displayDays) }}" class="text-left font-bold bg-gray-100 text-[9px] uppercase pl-2">FEMALE</td>
                    </tr>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 29 + count($displayDays) }}" class="text-center py-2 text-slate-400 text-[8px]">No female students</td>
                        </tr>
                    @endforelse

                    <!-- FEMALE TOTAL ROW -->
                    <tr class="bg-gray-50 font-bold">
                        <td colspan="{{ 2 + count($displayDays) + $remainingCols }}" class="text-right text-[9px] pr-2">FEMALE TOTAL PER DAY:</td>
                        @foreach($displayDays as $day)
                            @php
                                $dateStr = sprintf('%04d-%02d-%02d', $year, $monthNum, $day);
                            ...
                            @endphp
                            <td class="text-[8px]">{{ $femalePresent }}</td>
                        @endforeach
                        @for($i = 0; $i < $remainingCols; $i++)
                            <td class="text-[8px]"></td>
                        @endfor
                        <td class="text-[9px]">{{ $femaleTotalAbsent > 0 ? $femaleTotalAbsent : '' }}</td>
                        <td class="text-[9px]">{{ $femaleTotalTardy > 0 ? $femaleTotalTardy : '' }}</td>
                        <td></td>
                    </tr>

                    <!-- COMBINED TOTAL ROW -->
                    <tr class="bg-gray-200 font-bold border-t-2 border-black">
                        <td colspan="{{ 2 + count($displayDays) + $remainingCols }}" class="text-right text-[9px] pr-2">COMBINED TOTAL PER DAY:</td>
                        @foreach($displayDays as $day)
                            @php
                                $dateStr = sprintf('%04d-%02d-%02d', $year, $monthNum, $day);
                            ...
                            @endphp
                            <td class="text-[8px] border-b-2 border-black">{{ $totalPresent }}</td>
                        @endforeach
                        @for($i = 0; $i < $remainingCols; $i++)
                            <td class="text-[8px] border-b-2 border-black"></td>
                        @endfor
                        <td class="text-[9px] border-b-2 border-black">{{ ($maleTotalAbsent + $femaleTotalAbsent) > 0 ? ($maleTotalAbsent + $femaleTotalAbsent) : '' }}</td>
                        <td class="text-[9px] border-b-2 border-black">{{ ($maleTotalTardy + $femaleTotalTardy) > 0 ? ($maleTotalTardy + $femaleTotalTardy) : '' }}</td>
                    ...
                    @for($i = $currentRows; $i < $totalRows; $i++)
                        <tr style="height: 16px;">
                            <td class="text-center text-[8px]">{{ $enrollments->count() + ($i - $currentRows + 1) }}</td>
                            @for($j = 0; $j < (28 + count($displayDays) + $remainingCols); $j++)
                                <td></td>
                            @endfor
                        </tr>
                    @endfor
                    ...
                    <div class="summary-box bg-gray-50">
                        ...
                        <div class="grid grid-cols-2 gap-1 text-[8px]">
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold border-b border-black w-8 text-center">{{ $enrollments->count() }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>Late Enrollment during the month:</span>
                                <span class="font-bold border-b border-black w-8 text-center">{{ $lateEnrollments ?? '0' }}</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-bold border-b border-black w-8 text-center">{{ $averageDailyAttendance ?? '' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>% of Attendance for the month:</span>
                                <span class="font-bold border-b border-black w-8 text-center">{{ $attendancePercentage ?? '' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-[8px] border border-black">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="border border-black p-1 text-center">{{ $dropoutMale ?? '' }}</td>
                                <td class="border border-black p-1 text-center">{{ $dropoutFemale ?? '' }}</td>
                                <td class="border border-black p-1 text-center font-bold">{{ ($dropoutMale + $dropoutFemale) > 0 ? ($dropoutMale + $dropoutFemale) : '' }}</td>
                            </tr>
                            <tr>
                                <td class="border border-black p-1 font-semibold">Transferred out</td>
                                <td class="border border-black p-1 text-center">{{ $transferredOutMale ?? '' }}</td>
                                <td class="border border-black p-1 text-center">{{ $transferredOutFemale ?? '' }}</td>
                                <td class="border border-black p-1 text-center font-bold">{{ ($transferredOutMale + $transferredOutFemale) > 0 ? ($transferredOutMale + $transferredOutFemale) : '' }}</td>
                            </tr>
                            <tr>
                                <td class="border border-black p-1 font-semibold">Transferred in</td>
                                <td class="border border-black p-1 text-center">{{ $transferredInMale ?? '' }}</td>
                                <td class="border border-black p-1 text-center">{{ $transferredInFemale ?? '' }}</td>
                                <td class="border border-black p-1 text-center font-bold">{{ ($transferredInMale + $transferredInFemale) > 0 ? ($transferredInMale + $transferredInFemale) : '' }}</td>
                            </tr>
                        </tbody>
                    </table>
