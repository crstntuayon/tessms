...
<body class="bg-slate-100 min-h-screen">
    ...
    <div class="ml-72 p-6">
        ...
        <div class="mb-4 flex items-center justify-between no-print">
            ...
            <div class="flex gap-3">
                <div class="px-4 py-2 rounded-lg bg-indigo-50 border border-indigo-200 text-indigo-700 text-sm font-medium">
                    <i class="fas fa-graduation-cap mr-2"></i>SY {{ $activeSchoolYear?->name ?? now()->format('Y') }}
                </div>
            </div>
        </div>
        ...
        <div class="no-print mb-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            <form method="GET" action="{{ route('teacher.sf6') }}" class="flex items-end gap-4">
                <div class="flex-1">
        ...
        @if($promotionData->isEmpty() && $selectedSection)
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center no-print mb-4">
                ...
                <p class="text-blue-800 font-medium">No student data found for {{ $selectedSection->name }}</p>
            ...
            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-emerald-600">{{ $summaryStats['promoted_male'] + $summaryStats['promoted_female'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-amber-600">{{ $summaryStats['conditional_male'] + $summaryStats['conditional_female'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-rose-600">{{ $summaryStats['retained_male'] + $summaryStats['retained_female'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-blue-600">{{ $summaryStats['advanced_male'] + $summaryStats['advanced_female'] }}</p>
                    </div>
        ...
        <!-- SF6 Container -->
        <div class="sf6-container bg-white p-4 rounded-xl shadow-lg border border-slate-200 max-w-[1600px] mx-auto">
                ...
                <tbody>
                    ...
                    @forelse($maleData as $index => $data)
                        <tr>
                            ...
                            <td class="text-[9px] proficiency-{{ strtolower(str_replace(' ', '-', $data['proficiency_level'])) }}">
                                {{ $data['proficiency_level'] }}
                            </td>
                            <td class="text-[9px] {{ strtolower($data['promotion_status']) }}">
                    ...
                    <tr class="bg-gray-50 font-bold">
                        ...
                        <td class="text-[9px]">{{ $summaryStats['male_count'] }}</td>
                        <td class="text-[9px]">{{ round($maleData->avg('final_average')) }}</td>
                        <td class="text-[9px]"></td>
                        <td class="text-[9px]"></td>
                        <td class="text-[9px]">
                            P:{{ $summaryStats['promoted_male'] }} 
                            C:{{ $summaryStats['conditional_male'] }} 
                            R:{{ $summaryStats['retained_male'] }}
                        </td>
                        <td></td>
                    </tr>
                    ...
                    <tr class="bg-gray-50 font-bold">
                        ...
                        <td class="text-[9px]">{{ $summaryStats['female_count'] }}</td>
                        <td class="text-[9px]">{{ round($femaleData->avg('final_average')) }}</td>
                        <td class="text-[9px]"></td>
                        <td class="text-[9px]"></td>
                        <td class="text-[9px]">
                            P:{{ $summaryStats['promoted_female'] }} 
                            C:{{ $summaryStats['conditional_female'] }} 
                            R:{{ $summaryStats['retained_female'] }}
                        </td>
                        <td></td>
                    </tr>
                    ...
                    <tr class="bg-gray-200 font-bold border-t-2 border-black">
                        ...
                        <td class="text-[9px] border-b-2 border-black">{{ $summaryStats['total_students'] }}</td>
                        ...
                        <td class="text-[9px] border-b-2 border-black">
                            P:{{ $summaryStats['promoted_male'] + $summaryStats['promoted_female'] }} 
                            C:{{ $summaryStats['conditional_male'] + $summaryStats['conditional_female'] }} 
                            R:{{ $summaryStats['retained_male'] + $summaryStats['retained_female'] }}
                        </td>
                ...
                <table class="sf6-table" style="width: 80%; margin: 0 auto;">
                    ...
                    <tbody>
                        <tr class="proficiency-advanced">
                            ...
                            <td class="font-bold">{{ $summaryStats['advanced_male'] > 0 ? $summaryStats['advanced_male'] : '' }}</td>
                            <td class="font-bold">{{ $summaryStats['advanced_female'] > 0 ? $summaryStats['advanced_female'] : '' }}</td>
                            <td class="font-bold">{{ ($summaryStats['advanced_male'] + $summaryStats['advanced_female']) > 0 ? ($summaryStats['advanced_male'] + $summaryStats['advanced_female']) : '' }}</td>
                            <td class="text-[8px]">90 - 100</td>
                        </tr>
                        <tr class="proficiency-proficient">
                            <td class="text-left pl-2 font-bold">Proficient</td>
                            <td class="font-bold">{{ $summaryStats['proficient_male'] > 0 ? $summaryStats['proficient_male'] : '' }}</td>
                            <td class="font-bold">{{ $summaryStats['proficient_female'] > 0 ? $summaryStats['proficient_female'] : '' }}</td>
                            <td class="font-bold">{{ ($summaryStats['proficient_male'] + $summaryStats['proficient_female']) > 0 ? ($summaryStats['proficient_male'] + $summaryStats['proficient_female']) : '' }}</td>
                            <td class="text-[8px]">85 - 89</td>
                        </tr>
                        <tr class="proficiency-approaching">
                            <td class="text-left pl-2 font-bold">Approaching Proficiency</td>
                            <td class="font-bold">{{ $summaryStats['approaching_male'] > 0 ? $summaryStats['approaching_male'] : '' }}</td>
                            <td class="font-bold">{{ $summaryStats['approaching_female'] > 0 ? $summaryStats['approaching_female'] : '' }}</td>
                            <td class="font-bold">{{ ($summaryStats['approaching_male'] + $summaryStats['approaching_female']) > 0 ? ($summaryStats['approaching_male'] + $summaryStats['approaching_female']) : '' }}</td>
                            <td class="text-[8px]">80 - 84</td>
                        </tr>
                        <tr class="proficiency-developing">
                            <td class="text-left pl-2 font-bold">Developing</td>
                            <td class="font-bold">{{ $summaryStats['developing_male'] > 0 ? $summaryStats['developing_male'] : '' }}</td>
                            <td class="font-bold">{{ $summaryStats['developing_female'] > 0 ? $summaryStats['developing_female'] : '' }}</td>
                            <td class="font-bold">{{ ($summaryStats['developing_male'] + $summaryStats['developing_female']) > 0 ? ($summaryStats['developing_male'] + $summaryStats['developing_female']) : '' }}</td>
                            <td class="text-[8px]">75 - 79</td>
                        </tr>
                        <tr class="proficiency-beginning">
                            <td class="text-left pl-2 font-bold">Beginning</td>
                            <td class="font-bold">{{ $summaryStats['beginning_male'] > 0 ? $summaryStats['beginning_male'] : '' }}</td>
                            <td class="font-bold">{{ $summaryStats['beginning_female'] > 0 ? $summaryStats['beginning_female'] : '' }}</td>
                            <td class="font-bold">{{ ($summaryStats['beginning_male'] + $summaryStats['beginning_female']) > 0 ? ($summaryStats['beginning_male'] + $summaryStats['beginning_female']) : '' }}</td>
