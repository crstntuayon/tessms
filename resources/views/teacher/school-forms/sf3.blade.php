...
<head>
    ...
    <title>SF3 - Books Issued and Returned - {{ $selectedSection->name ?? '' }}</title>
...
<div class="flex">
    ...
    <div class="ml-72 w-full min-h-screen p-8">
        ...
        <!-- SF3 Form Container -->
        <div class="sf3-container bg-white p-8 shadow-lg border border-slate-200">
            ...
            <table class="w-full sf3-table text-xs border-collapse border-2 border-slate-800">
                <thead>
                    <tr class="bg-slate-200">
                        ...
                        @foreach($bookInventories as $index => $inventory)
                                    ...
                                    <td class="border border-slate-600 p-1 text-center text-xs">
                                        @if($book)
                                            @if($book->status == 'lost')
                                                ...
                                                @if($book->loss_code)<br>({{ $book->loss_code }})@endif
                                            @elseif($book->status == 'damaged')
                                                <span class="text-amber-600">Damaged</span>
                                            @elseif($book->date_returned)
        ...
        <div class="mt-6 flex gap-3 no-print">
            <a href="{{ route('teacher.books.issue', $selectedSection) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
                <i class="fas fa-plus mr-2"></i>Issue Book
            </a>
            <a href="{{ route('teacher.books.return', $selectedSection) }}" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
                <i class="fas fa-undo mr-2"></i>Return Book
            </a>
            <a href="{{ route('teacher.books.inventory') }}" class="px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">
