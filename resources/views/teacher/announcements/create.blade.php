...
<head>
    ...
    <title>Post Announcement - Teacher Portal</title>
    ...
    <div class="flex-1 lg:ml-72 h-screen flex flex-col bg-slate-50 overflow-hidden">
        ...
        <div class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-0.5">Create and broadcast an announcement to your students</p>
            </div>
            <a href="{{ route('teacher.announcements.index') }}" class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
        ...
        <div class="flex-1 overflow-y-auto p-6">
            <div class="max-w-3xl mx-auto">
                <form action="{{ route('teacher.announcements.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden" @submit="return validateForm()">
                    @csrf
                    ...
                    <div class="p-6 border-b border-slate-100">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="scope" x-model="scope" @change="onScopeChange()" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm bg-white">
                            ...
                            <div x-show="scope === 'section'" x-transition>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Select Section <span class="text-rose-500">*</span></label>
                                <select name="target_id" x-model="targetId" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm bg-white">
                            ...
                            <div x-show="scope === 'grade'" x-transition>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">Select Grade Level <span class="text-rose-500">*</span></label>
                                <select name="target_id" x-model="targetId" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm bg-white">
                    ...
                    <div class="p-6 bg-slate-50 flex items-center justify-end gap-3">
                        <a href="{{ route('teacher.announcements.index') }}" class="px-6 py-2.5 text-sm font-medium text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                            Cancel
                        </a>
    ...
    return {
        ...
        get priorityLabel() {
            const labels = {
                ...
                important: 'This is marked as important. It will be highlighted to students.',
                urgent: 'This is marked as urgent. Students will see a prominent alert.',
            };
        ...
        validateForm() {
            ...
            if ((this.scope === 'section' || this.scope === 'grade') && !this.targetId) {
                alert('Please select a target section or grade level.');
                return false;
            }
            return true;
        }
