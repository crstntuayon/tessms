                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-slate-900 capitalize">{{ $announcement->scope }}</span>
                        </div>
                        @if($announcement->scope === 'section' && $announcement->section)
                            <div class="flex justify-between">
                                <span class="text-slate-500">Target Section</span>
                                <span class="font-medium text-slate-900">{{ $announcement->section->name }} ({{ $announcement->section->gradeLevel->name ?? 'N/A' }})</span>
                            </div>
                        @endif
                        @if($announcement->scope === 'grade' && $announcement->gradeLevel)
                            <div class="flex justify-between">
                                <span class="text-slate-500">Target Grade</span>
                                <span class="font-medium text-slate-900">{{ $announcement->gradeLevel->name }}</span>
                            </div>
                        @endif
                        <div class="flex justify-between">
