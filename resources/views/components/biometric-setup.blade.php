...
<div x-data="biometricSetup()" x-init="init()" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    ...
    <div x-show="checking" class="text-center py-8">
        ...
        <p class="text-slate-600">Checking device compatibility...</p>
    </div>
    ...
    <div x-show="!isAvailable && !checking" class="text-center py-6">
        ...
        <p class="text-slate-600 mb-2">Biometric authentication is not available on this device.</p>
    ...
    <div x-show="isAvailable && !checking">
        ...
        <div x-show="hasCredentials" class="space-y-4">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-green-600">
                            You can now log in using Face ID or fingerprint
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <template x-for="cred in credentials" :key="cred.id">
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            ...
                            <button @click="removeCredential(cred.id)" 
                                ...
                                <i x-show="removing === cred.id" class="fas fa-spinner fa-spin"></i>
                            </button>
                        </div>
                    </template>
                </div>
            ...
            <button @click="startRegistration()" 
                ...
                <i x-show="registering" class="fas fa-spinner fa-spin"></i>
        ...
        <div x-show="!hasCredentials">
            <p class="text-slate-600 mb-4">
                Set up biometric authentication to log in quickly and securely using your Face ID or fingerprint.
            </p>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center space-x-3 text-sm text-slate-600">
                    <i class="fas fa-check text-green-500"></i>
                    <span>Enhanced security with device biometrics</span>
                </div>
                <div class="flex items-center space-x-3 text-sm text-slate-600">
                    <i class="fas fa-check text-green-500"></i>
            ...
            <button @click="startRegistration()" 
                ...
                <i x-show="registering" class="fas fa-spinner fa-spin"></i>
                <span x-text="registering ? 'Setting up...' : 'Set Up Biometric Login'"></span>
            </button>
        </div>
    </div>
    ...
    <div x-show="showRegistrationModal" 
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl">
            ...
            <p class="text-slate-600 mb-4">
                Use your device's Face ID or fingerprint sensor to complete setup.
            </p>
    ...
    return {
        ...
        async startRegistration() {
            ...
            try {
                ...
                if (result.success) {
                    await this.loadCredentials();
                    this.showToast('Biometric login set up successfully!', 'success');
                }
            } catch (error) {
                console.error('Registration error:', error);
                this.errorMessage = error.message || 'Failed to set up biometric authentication';
            } finally {
        ...
        async removeCredential(id) {
            if (!confirm('Remove this device? You will need to set up biometric login again.')) {
                return;
            }
