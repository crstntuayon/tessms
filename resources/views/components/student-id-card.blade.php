...
$user = $student->user;
$photoUrl = ($user?->photo ?? null) ? asset('storage/' . $user->photo) : null;
...
<div class="id-card-wrapper flex flex-col md:flex-row items-center justify-center gap-8">
    ...
    <div class="id-card relative bg-white rounded-xl shadow-2xl overflow-hidden flex" style="width: 2.125in; height: 3.375in;">
        ...
        <div class="w-10 bg-blue-900 flex flex-col items-center justify-between shrink-0 py-1">
            @foreach(str_split('STUDENT') as $letter)
                <span class="text-white text-[22px] font-black leading-none">{{ $letter }}</span>
            @endforeach
        </div>
        ...
        <div class="flex-1 flex flex-col items-center p-2 pt-6">
            ...
            <div class="mt-2">
                @if($photoUrl)
                    <img src="{{ $photoUrl }}" alt="Student Photo" class="w-[0.9in] h-[1.1in] object-cover rounded-md border border-slate-200 shadow-sm bg-slate-100">
                @else
            ...
            <div class="text-center mt-2 w-full px-1">
                ...
                <p class="text-[10px] font-bold text-slate-900 leading-tight mt-0.5 truncate">{{ strtoupper($student->full_name) }}</p>
            </div>
            ...
            <div class="text-center mt-1 w-full px-1">
                ...
                <p class="text-[12px] font-black text-blue-900 tracking-wide leading-tight mt-0.5">{{ $lrn }}</p>
            </div>
            ...
            <div class="mt-auto w-full flex items-end justify-between pt-1">
                <div>
                    ...
                    <p class="text-[9px] font-bold text-slate-700">{{ $schoolId }}</p>
                </div>
                <div class="text-center">
                    <img src="data:image/png;base64,{{ $qrBase64 }}" alt="QR" class="w-9 h-9 rounded border border-slate-100 shadow-sm">
        ...
        <div class="px-2.5 py-2 flex flex-col h-[calc(3.375in-2.375rem)]">
            ...
            <div>
                ...
                <div class="mt-1 flex gap-2">
                    <div class="flex-1">
                        ...
                        <p class="text-[8px] font-bold text-slate-800 truncate">{{ $emergencyName }}</p>
                    </div>
                    <div class="flex-1">
                        <p class="text-[5px] text-slate-500 uppercase font-semibold">Contact No.</p>
                        <p class="text-[8px] font-bold text-slate-800 truncate">{{ $emergencyPhone }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-1.5 flex flex-col items-center justify-center flex-1">
                ...
                <div class="mt-2 text-center flex flex-col items-center">
                    <p class="text-[9px] font-bold text-slate-800">{{ $principalName }}</p>
